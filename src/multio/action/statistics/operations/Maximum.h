...
class Maximum final : public OperationWithData<ComputationalType, 1> {
public:
    ...
    Maximum(const std::string& name, long sz, const OperationWindow& win, const StatisticsConfiguration& cfg) :
        OperationWithData<ComputationalType, 1>{name, "maximum", sz / sizeof(InputOutputType), true, win, cfg} {}


    Maximum(const std::string& name, long sz, const OperationWindow& win, std::shared_ptr<StatisticsIO>& IOmanager,
            const StatisticsConfiguration& cfg) :
        OperationWithData<ComputationalType, 1>{name,      "maximum", sz / sizeof(InputOutputType), true, win,
                                                IOmanager, cfg} {};
