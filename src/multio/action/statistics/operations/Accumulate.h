...
private:
    void computeWithMissing(Q* buf) const {
        ...
        std::transform(values_.cbegin(), values_.cend(), buf,
                       [m](const state& v) { return static_cast<Q>(m == v[0] ? m : v[0]); });
        return;
    }
    ...
    void updateWithoutMissing(const Q* val) {
        std::transform(values_.cbegin(), values_.cend(), val, values_.begin(),
                       [](const state& v1, const Q& v2) { return state{static_cast<T>(v1[0] + v2)}; });
        return;
    }
    ...
    void updateWithMissing(const Q* val) {
        ...
        std::transform(values_.cbegin(), values_.cend(), val, values_.begin(),
                       [m](const state& v1, const Q& v2) { return state{static_cast<T>(m == v2 ? m : v1[0] + v2)}; });
        return;
    }
