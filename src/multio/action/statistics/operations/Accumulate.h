...
public:
    ...
    Accumulate(const std::string& name, long sz, StatisticsOptions& options) :
        OperationWithData<T>{name, "accumulate", sz, options}{}

    Accumulate(const std::string& name, long sz, const std::string& partialPath, StatisticsOptions& options) :
        OperationWithData<T>{name, "accumulate", sz, partialPath, options}{ };
...
private:
    void updateWithoutMissing( const T* val ){ 
        std::transform( values_.begin(), values_.end(), val, values_.begin(), [](T v1, T v2){ return static_cast<T>(v1 + v2); } );
        return;
    }
    void updateWithMissing( const T* val ){
        double m=options_.missingValue();   
        std::transform( values_.begin(), values_.end(), val, values_.begin(), [m](T v1, T v2){ return static_cast<T>(m==v2 ? m : v1 + v2); } );
        return;        
    }
