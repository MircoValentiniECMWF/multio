...
CONTAINS
  ...
  !> the GRIB2 encoder.
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: INIT => GRIB2_SECTION3_050_INIT_LAZY
  ...
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: PRINT => GRIB2_SECTION3_050_PRINT
...
PP_THREAD_SAFE FUNCTION GRIB2_SECTION3_050_PRESET( HIS, &
&  MSG, PAR, OPT,  METADATA, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Set the frequency numbers
  PP_METADATA_SET( METADATA,  'J', PAR%GEO%SH(IDX)%NFREQ )
  PP_METADATA_SET( METADATA,  'K', PAR%GEO%SH(IDX)%NFREQ )
  PP_METADATA_SET( METADATA,  'M', PAR%GEO%SH(IDX)%NFREQ )

  ! Details of the spectral representation
  PP_METADATA_SET( METADATA,  'spectralMode', 1 )
  PP_METADATA_SET( METADATA,  'spectralType', 1 )
...
PP_THREAD_SAFE FUNCTION GRIB2_SECTION3_050_TO_BE_ENCODED( HIS, &
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION3_050_PRINT'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION3_050_PRINT( THIS, &
...
IMPLICIT NONE
  ...
  IF ( OFFSET .LE. 0 ) THEN
    WRITE(UNIT,'(A,A,A,A)', IOSTAT=WRITE_STAT) 'GRIB2_SECTION3_050_T: ', TRIM(ADJUSTL(THIS%NUMBER_)), '::', TRIM(ADJUSTL(THIS%TYPE_))
    PP_DEBUG_CRITICAL_COND_THROW(WRITE_STAT.NE.0, ERRFLAG_WRITE_ERROR )
  ELSE
    WRITE(UNIT,'(A,A,A,A,A)', IOSTAT=WRITE_STAT) REPEAT(' ',OFFSET), 'GRIB2_SECTION3_050_T: ', TRIM(ADJUSTL(THIS%NUMBER_)), '::', TRIM(ADJUSTL(THIS%TYPE_))
    PP_DEBUG_CRITICAL_COND_THROW(WRITE_STAT.NE.0, ERRFLAG_WRITE_ERROR )
  ENDIF
...
END FUNCTION GRIB2_SECTION3_050_PRINT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
