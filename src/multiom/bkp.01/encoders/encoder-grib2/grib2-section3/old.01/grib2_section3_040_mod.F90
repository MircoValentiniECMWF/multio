...
CONTAINS
  ...
  !> the GRIB2 encoder.
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: INIT => GRIB2_SECTION3_040_INIT_LAZY
  ...
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: PRINT => GRIB2_SECTION3_040_PRINT
...
!>
!> This function initializes a GRIB2 Section 3 object (`THIS`) using YAML configuration data (`CFG`).
!>
!> @section interface
...
!> @brief Initializes GRIB2 Section 3 for a given object using the provided parameters.
!>
!> This function initializes a GRIB2 Section 3 object (`THIS`) using the provided model message (`MSG`)
...
IMPLICIT NONE
  ...
  PP_METADATA_SET( METADATA, 'numberOfOctectsForNumberOfPoints', 2 )

  !> MEaning of the description array
  PP_METADATA_SET( METADATA, 'interpretationOfNumberOfPoints', 1 )

  ! Set the number of local definitions
  PP_METADATA_SET( METADATA,  'gridDefinitionTemplateNumber', 40 )
  ...
  PP_METADATA_SET( METADATA, 'pl', PAR%GEO%GG(IDX)%PL )
...
PP_THREAD_SAFE FUNCTION GRIB2_SECTION3_040_PRESET( HIS, &
&  MSG, PAR, OPT,  METADATA, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  PP_METADATA_SET( METADATA,  'shapeOfTheEarth', 6 )
  PP_METADATA_SET_MISSING( METADATA, 'scaleFactorOfRadiusOfSphericalEarth' )
  PP_METADATA_SET_MISSING( METADATA, 'scaledValueOfRadiusOfSphericalEarth' )
  PP_METADATA_SET_MISSING( METADATA, 'scaleFactorOfEarthMajorAxis' )
  PP_METADATA_SET_MISSING( METADATA, 'scaledValueOfEarthMajorAxis' )
  PP_METADATA_SET_MISSING( METADATA, 'scaleFactorOfEarthMinorAxis' )
  PP_METADATA_SET_MISSING( METADATA, 'scaledValueOfEarthMinorAxis' )

  ! Preset grid size
  PP_METADATA_SET_MISSING( METADATA, 'Ni' )
  PP_METADATA_SET( METADATA,  'Nj', NLAT ) ! numberOfPointsAlongAMeridian

  ! Preset grid resolution
  PP_METADATA_SET( METADATA, 'basicAngleOfTheInitialProductionDomain', 0 )
  PP_METADATA_SET_MISSING( METADATA, 'subdivisionsOfBasicAngle' )


  ! Preset grid spacing
  PP_METADATA_SET( METADATA,  'latitudeOfFirstGridPointInDegrees', LAT_FIRST_GP_DEG )
  PP_METADATA_SET( METADATA,  'longitudeOfFirstGridPointInDegrees', LON_FIRST_GP_DEG )
  ...
  PP_METADATA_SET_MISSING( METADATA, 'iDirectionIncrement' )
  PP_METADATA_SET( METADATA,  'N', NLAT_BETWEEN_POLE_AND_EQUATOR ) ! numberOfParallelsBetweenAPoleAndTheEquator

  ! Reset the pl array another time just to be sure (in any case it is done at the beginning of the simulation)
  PP_METADATA_SET( METADATA, 'scanningMode', 0 )
  PP_METADATA_SET( METADATA, 'pl', PAR%GEO%GG(IDX)%PL )
...
PP_THREAD_SAFE FUNCTION GRIB2_SECTION3_040_TO_BE_ENCODED( HIS, &
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION3_040_PRINT'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION3_040_PRINT( THIS, &
...
IMPLICIT NONE
  ...
  IF ( OFFSET .LE. 0 ) THEN
    WRITE(UNIT,'(A,A,A,A)', IOSTAT=WRITE_STAT) 'GRIB2_SECTION3_040_T: ', TRIM(ADJUSTL(THIS%NUMBER_)), '::', TRIM(ADJUSTL(THIS%TYPE_))
    PP_DEBUG_CRITICAL_COND_THROW(WRITE_STAT.NE.0, ERRFLAG_WRITE_ERROR )
  ELSE
    WRITE(UNIT,'(A,A,A,A,A)', IOSTAT=WRITE_STAT) REPEAT(' ',OFFSET), 'GRIB2_SECTION3_040_T: ', TRIM(ADJUSTL(THIS%NUMBER_)), '::', TRIM(ADJUSTL(THIS%TYPE_))
    PP_DEBUG_CRITICAL_COND_THROW(WRITE_STAT.NE.0, ERRFLAG_WRITE_ERROR )
  ENDIF
...
END FUNCTION GRIB2_SECTION3_040_PRINT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
