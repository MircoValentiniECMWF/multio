...
#define PP_SECTION_NAME 'G2S4_CLOUDBASE_MOD'
MODULE G2S4_CLOUDBASE_MOD
...
!>
!> The `G2S4_CLOUDBASE_T` type extends the base class `GRIB_SECTION_BASE_A` and
...
!> environments.
!>
TYPE, EXTENDS(GRIB_SECTION_BASE_A) :: G2S4_CLOUDBASE_T
...
CONTAINS
  ...
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: ALLOC => G2S4_CLOUDBASE_ALLOC
  ...
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: RT => G2S4_CLOUDBASE_RT
  ...
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: TBE => G2S4_CLOUDBASE_TBE
...
!>
!> @section interface
!>   @param [inout] THIS  An object of type `G2S4_CLOUDBASE_T` representing the GRIB section being initialized.
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(G2S4_CLOUDBASE_T),  INTENT(INOUT) :: THIS
...
!>
!> @section interface
!>   @param [in]    THIS     An object of type `G2S4_CLOUDBASE_T` representing the GRIB section to allocate resources for.
!>   @param [in]    MSG      All the mars keywords needed to describe the field `FORTRAN_MESSAGE_T`.
!>   @param [in]    PAR      All information outside mars keywords needed to describe the field `PARAMETRIZATION_T`.
!>   @param [in]    OPT      The encoder options structure of type `ENCODER_OPTIONS_T`.
...
!>
!> @section interface
!>   @param [in]    THIS     An object of type `G2S4_CLOUDBASE_T` representing the GRIB section to be preset.
...
!>
!> @section interface
!>   @param [in]    THIS      An object of type `G2S4_CLOUDBASE_T` representing the GRIB section for runlevel execution.
!>   @param [in]    MSG       The message object of type `FORTRAN_MESSAGE_T` used to handle preset-related messaging.
!>   @param [in]    PAR       The parametrization structure of type `PARAMETRIZATION_T` used for the preset operation.
...
!>
!> @section interface
!>   @param [inout] THIS          An object of type `G2S4_CLOUDBASE_T` representing the GRIB section being checked.
!>   @param [in]    MSG           The message object of type `FORTRAN_MESSAGE_T` used to handle preset-related messaging.
!>   @param [in]    PAR           The parametrization structure of type `PARAMETRIZATION_T` used for the preset operation.
!>   @param [in]    TIME_HIST     The time history object of type `TIME_HISTORY_T` providing historical level data.
...
!>
!> @section interface
!>   @param [inout] THIS  An object of type `G2S4_CLOUDBASE_T` representing the GRIB section to be freed.
!>   @param [in]    OPT   The encoder options structure of type `ENCODER_OPTIONS_T`.
