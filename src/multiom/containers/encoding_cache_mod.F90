...
INTERFACE
  PP_THREAD_SAFE FUNCTION FUNCTION_ENCODING_CACHE_IF( KEY, OPT, HOOKS ) RESULT(RET)
    ! Symbols imported from other modules within the project.
    USE :: DATAKINDS_DEF_MOD,        ONLY: JPIB_K
...
ABSTRACT INTERFACE
  PP_THREAD_SAFE FUNCTION FUNCTOR_ENCODING_CACHE_IF( THIS, KEY, OPT,  HOOKS ) RESULT(RET)
    ! Symbols imported from other modules within the project.
    USE :: DATAKINDS_DEF_MOD,        ONLY: JPIB_K
    USE :: HOOKS_MOD,                ONLY: HOOKS_T
    USE :: GRIB_ENCODER_OPTIONS_MOD, ONLY: GRIB_ENCODER_OPTIONS_T
...
TYPE :: ENCODING_CACHE_NODE_T
  ...
  !> Value
  TYPE(CACHED_ENCODER_COLLECTION_T) :: ENCODERS
...
PP_THREAD_SAFE FUNCTION KEY_EQUAL_TO( KEY1, KEY2, OPT, IS_EQUAL, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  PP_TRYCALL(ERRFLAG_COMPARE_ERROR) KEY1%IS_EQUAL_TO( KEY2, OPT, IS_EQUAL, HOOKS )
...
PP_THREAD_SAFE FUNCTION KEY_LOWER_THAN( KEY1, KEY2, OPT, IS_LOWER_THAN, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  PP_TRYCALL(ERRFLAG_COMPARE_ERROR) KEY1%IS_LOWER_THAN( KEY2, OPT, IS_LOWER_THAN, HOOKS )
...
&                                        ENCODERS, INSERTED, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_ISLEAF) NODE_ISLEAF( ROOT, ROOT_IS_LEAF, OPT, HOOKS )
  IF ( ROOT_IS_LEAF ) THEN

  ! Map is empty
    INSERTED =.TRUE.
    PP_TRYCALL(ERRFLAG_UNABLE_TO_ALLOCATE_NODE) ALLOCATE_NODE( ROOT, OPT, HOOKS )
    INSERTION_POINT => NIL
    PP_TRYCALL(ERRFLAG_UNABLE_TO_INITIALIZE_NODE) NODE_INIT( ROOT, INSERTION_POINT, KEY, &
&       PAR, METADATA, ENCODING_RULES, OPT, ENCODERS, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_FIXUP_INSERT) INSERT_FIXUP( ROOT, ROOT, OPT, HOOKS )
    INSERTION_POINT => ROOT
  ELSE

    ! Map not empty
    INSERTION_POINT => NIL
    PP_TRYCALL(ERRFLAG_UNABLE_TO_SEARCH_NODE) SEARCH_NODE( ROOT, INSERTION_POINT, KEY, OPT, FOUND, HOOKS )

    IF ( .NOT.FOUND ) THEN
      ! If the node is not in the map then insert it
      INSERTED =.TRUE.
      PP_TRYCALL(ERRFLAG_KEY_LT_FAILURE) KEY_LOWER_THAN( KEY, INSERTION_POINT%KEY, OPT, KEY_LT, HOOKS )
      IF ( KEY_LT ) THEN
        PP_TRYCALL(ERRFLAG_UNABLE_TO_ALLOCATE_NODE) ALLOCATE_NODE( INSERTION_POINT%LEFT, OPT, HOOKS )
        PP_TRYCALL(ERRFLAG_UNABLE_TO_INITIALIZE_NODE) NODE_INIT( INSERTION_POINT%LEFT, INSERTION_POINT, &
&             KEY, PAR, METADATA, ENCODING_RULES, OPT, ENCODERS, HOOKS )
        PP_TRYCALL(ERRFLAG_UNABLE_TO_FIXUP_INSERT) INSERT_FIXUP( ROOT, INSERTION_POINT%LEFT, OPT, HOOKS )
      ELSE
        PP_TRYCALL(ERRFLAG_UNABLE_TO_INITIALIZE_NODE) ALLOCATE_NODE( INSERTION_POINT%RIGHT, OPT, HOOKS )
        PP_TRYCALL(ERRFLAG_UNABLE_TO_ALLOCATE_NODE) NODE_INIT( INSERTION_POINT%RIGHT, INSERTION_POINT, &
&             KEY, PAR, METADATA, ENCODING_RULES, OPT, ENCODERS, HOOKS )
        PP_TRYCALL(ERRFLAG_UNABLE_TO_FIXUP_INSERT) INSERT_FIXUP( ROOT, INSERTION_POINT%RIGHT, OPT, HOOKS )
      ENDIF
    ELSE
      ! If the node is in the map then return the value
      INSERTED =.FALSE.
      ENCODERS => INSERTION_POINT%ENCODERS
    ENDIF
...
PP_THREAD_SAFE FUNCTION DELETE_NODE( ROOT, KEY, OPT, FOUND, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  IF ( .NOT.ROOT_IS_LEAF ) THEN
    PP_TRYCALL(ERRFLAG_ERRFLAG_UNABLE_TO_SEARCH_NODE) SEARCH_NODE( ROOT, INSERTION_POINT, KEY, OPT, FOUND, HOOKS )
    IF ( FOUND ) THEN
      PP_TRYCALL(ERRFLAG_UNABLE_TO_REMOVE_NODE) REMOVE_NODE( ROOT, INSERTION_POINT, OPT, HOOKS )
    ENDIF
  ENDIF
...
RECURSIVE FUNCTION FREE_NODE( CURRENT, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  IF ( .NOT.ASSOCIATED( CURRENT, NIL ) ) THEN
    ...
    PP_TRYCALL(ERRFLAG_UNABLE_TO_FREE_NODE) FREE_NODE( CURRENT%LEFT, OPT, HOOKS )

    ! Deallocate right subtree.
    PP_TRYCALL(ERRFLAG_UNABLE_TO_FREE_NODE) FREE_NODE( CURRENT%RIGHT, OPT, HOOKS )

    ! Free memory
    PP_TRYCALL(ERRFLAG_UNABLE_TO_DEALLOCATE_NODE) DEALLOCATE_NODE( CURRENT, OPT, HOOKS )
...
PP_THREAD_SAFE FUNCTION SEARCH_NODE( ROOT, CURRENT, KEY, OPT, FOUND, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ELSE
    ...
    SearchLoop: DO
      ...
      PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_ISLEAF) NODE_ISLEAF( CURRENT, IS_LEAF, OPT, HOOKS )
      PP_TRYCALL(ERRFLAG_KEY_EQ_FAILURE) KEY_EQUAL_TO( KEY, CURRENT%KEY, OPT, KEY_EQ, HOOKS )
      ...
      ! Left subtree
      PP_TRYCALL(ERRFLAG_KEY_LT_FAILURE) KEY_LOWER_THAN( KEY, CURRENT%KEY, OPT, KEY_LT, HOOKS )
      IF ( KEY_LT ) THEN

        !> Check if the current node is a leaf
        PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_ISLEAF) NODE_ISLEAF( CURRENT%LEFT, IS_LEAF, OPT, HOOKS )
      ...
      ELSE
        ...
        !> Check if the current node is a leaf
        PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_ISLEAF) NODE_ISLEAF( CURRENT%RIGHT, IS_LEAF, OPT, HOOKS )
...
PP_THREAD_SAFE FUNCTION NODE_INIT( THIS, PARENT, KEY, PAR, &
        METADATA, ENCODING_RULES, OPT, ENCODERS, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Key initialization
  PP_TRYCALL(ERRFLAG_COPY_KEY_FAILURE) THIS%KEY%COPY_FROM( KEY, OPT, HOOKS )

  !> Encoder initialization
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CREATE_ENCODERS) THIS%ENCODERS%INIT( KEY, PAR, METADATA, &
&   ENCODING_RULES, OPT, HOOKS )
...
PP_THREAD_SAFE FUNCTION ALLOCATE_NODE( CURRENT, OPT, HOOKS ) RESULT(RET)
...
PP_THREAD_SAFE FUNCTION DEALLOCATE_NODE( X, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  IF ( ASSOCIATED( X ) ) THEN
    ...
    PP_TRYCALL(ERRFLAG_UNABLE_TO_FREE_KEY) X%KEY%FREE( OPT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_FREE_VAL) X%ENCODERS%FREE( OPT, HOOKS )
...
PP_THREAD_SAFE FUNCTION INSERT_FIXUP( ROOT, CUR, OPT, HOOKS ) RESULT(RET)
    ...
    IF ( ASSOCIATED( X%PARENT%PARENT%LEFT, X%PARENT ) ) THEN
      ...
      ELSE
        ...
        IF ( ASSOCIATED( X, X%PARENT%RIGHT ) ) THEN
          ...
          PP_TRYCALL(ERRFLAG_UNABLE_TO_ROTATE_LEFT) ROTATE_LEFT( ROOT, X, OPT, HOOKS )
        ...
        PP_TRYCALL(ERRFLAG_UNABLE_TO_ROTATE_RIGHT) ROTATE_RIGHT( ROOT, X%PARENT%PARENT, OPT, HOOKS )
    ...
    ELSE
      ...
      ELSE
        ...
        IF ( ASSOCIATED( X, X%PARENT%LEFT ) ) THEN
          ...
          PP_TRYCALL(ERRFLAG_UNABLE_TO_ROTATE_RIGHT) ROTATE_RIGHT( ROOT, X, OPT, HOOKS )
        ...
        PP_TRYCALL(ERRFLAG_UNABLE_TO_ROTATE_LEFT) ROTATE_LEFT( ROOT, X%PARENT%PARENT, OPT, HOOKS )
...
PP_THREAD_SAFE FUNCTION ROTATE_LEFT( ROOT, X_, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  Y => X%RIGHT
  X%RIGHT => Y%LEFT
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_ISLEAF) NODE_ISLEAF( Y%LEFT, IS_LEAF, OPT, HOOKS )
  IF ( .NOT.IS_LEAF ) THEN
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_ISLEAF) NODE_ISLEAF( X%PARENT, IS_ROOT, OPT, HOOKS )
  IF ( IS_ROOT ) THEN
...
PP_THREAD_SAFE FUNCTION ROTATE_RIGHT( ROOT, X_, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  Y => X%LEFT
  X%LEFT => Y%RIGHT
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_ISLEAF) NODE_ISLEAF( Y%RIGHT, IS_LEAF, OPT, HOOKS )
  IF ( .NOT.IS_LEAF ) THEN
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_ISLEAF) NODE_ISLEAF( X%PARENT, IS_ROOT_LEAF, OPT, HOOKS )
...
PP_THREAD_SAFE FUNCTION NODE_ISLEAF( X, ISLEAF, OPT, HOOKS ) RESULT(RET)
...
PP_THREAD_SAFE FUNCTION SUCCESSOR( X, Y, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Check.
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_ISLEAF) NODE_ISLEAF( X, IS_LEAF, OPT, HOOKS )
  PP_DEBUG_CRITICAL_COND_THROW( IS_LEAF, ERRFLAG_NODE_IS_LEAF )

  ! Search cycle.
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_ISLEAF) NODE_ISLEAF( X%RIGHT, IS_LEAF, OPT, HOOKS )
  IF ( .NOT.IS_LEAF ) THEN

    ! If the node has a right child then the successor is the
    ! minimum of the  right subtree of the node.
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_MINIMUM) MINIMUM( X%RIGHT, Y, OPT, HOOKS )
  ...
  ELSE
    ...
    SearchSuccessor: DO
      ...
      PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_ISLEAF) NODE_ISLEAF( Y, IS_LEAF, OPT, HOOKS )
...
PP_THREAD_SAFE FUNCTION PREDECESSOR( X, Y, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Search cycle.
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_ISLEAF) NODE_ISLEAF( X%LEFT, IS_LEAF, OPT, HOOKS )
  IF ( .NOT.IS_LEAF ) THEN

    ! If the node has a left child then the successor is the
    ! minimum of the  left subtree of the node.
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_MAXIMUM) MAXIMUM( X%LEFT, Y, OPT, HOOKS )
...
PP_THREAD_SAFE FUNCTION MINIMUM( X, Y, OPT, HOOKS ) RESULT(RET)
...
PP_THREAD_SAFE FUNCTION MAXIMUM( X, Y, OPT, HOOKS ) RESULT(RET)
...
PP_THREAD_SAFE FUNCTION SWAP_DATA( NODE_1, NODE_2, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  PP_TRYCALL(ERRFLAG_SWAP_KEYS) NODE_1%KEY%SWAP_DATA( NODE_2%KEY, OPT, HOOKS )

  ! Swapping values between two nodes
  PP_TRYCALL(ERRFLAG_SWAP_VALUES) NODE_1%ENCODERS%SWAP_DATA( NODE_2%ENCODERS, OPT, HOOKS )
...
PP_THREAD_SAFE FUNCTION REMOVE_NODE( ROOT, Z, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Remove the node
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_ISLEAF) NODE_ISLEAF( Y%LEFT,  IS_LEAF_LEFT,  OPT, HOOKS )
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_ISLEAF) NODE_ISLEAF( Y%RIGHT, IS_LEAF_RIGHT, OPT, HOOKS )
  ...
  ELSE
    ...
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_SUCESSOR) SUCCESSOR( Z, Y, OPT, HOOKS )
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_ISLEAF) NODE_ISLEAF( Y%PARENT,  IS_LEAF, OPT, HOOKS )
  IF ( IS_LEAF ) THEN
  ...
  IF ( .NOT.ASSOCIATED( Z, Y ) ) THEN
    ...
    ! Copy data.
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_SWAP_DATA) SWAP_DATA( Y, Z, OPT, HOOKS )
  ...
  IF ( RED_BLACK_BALANCING ) THEN
    ...
    IF ( .NOT.Y%RED ) THEN
      ...
      PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_REMOVE_FIXUP) REMOVE_NODE_FIXUP( ROOT, X, OPT, HOOKS )
  ...
  ! Free memory.
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_DEALLOCATE_NODE) DEALLOCATE_NODE( Y, OPT, HOOKS )
  Y => NULL()
...
PP_THREAD_SAFE FUNCTION REMOVE_NODE_FIXUP( ROOT, X, OPT, HOOKS ) RESULT(RET)
  ...
  ColorFixupLoop: DO WHILE( .NOT.ASSOCIATED( X, ROOT ) .AND. .NOT.X%RED )
    ...
    IF ( ASSOCIATED( X, X%PARENT%LEFT ) ) THEN
      ...
      IF ( W%RED ) THEN
        ...
        PP_TRYCALL(ERRFLAG_UNABLE_TO_ROTATE_LEFT) ROTATE_LEFT(ROOT, X%PARENT, OPT, HOOKS )
      ...
      PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_ISLEAF) NODE_ISLEAF( W, IS_LEAF, OPT, HOOKS )
      PP_DEBUG_CRITICAL_COND_THROW( IS_LEAF, ERRFLAG_NODE_IS_LEAF )
      ...
      ELSE
        ...
        IF ( .NOT.W%RIGHT%RED ) THEN ! CASO 3
          ...
          PP_TRYCALL(ERRFLAG_UNABLE_TO_ROTATE_RIGHT) ROTATE_RIGHT(ROOT, W, OPT, HOOKS )
    ...
    ELSE ! Right child
      ...
      IF ( W%RED ) THEN
        ...
        PP_TRYCALL(ERRFLAG_UNABLE_TO_ROTATE_RIGHT)  ROTATE_RIGHT(ROOT, X%PARENT, OPT, HOOKS )
      ...
      ELSE
        ...
        IF ( .NOT.W%LEFT%RED ) THEN
          ...
          PP_TRYCALL(ERRFLAG_UNABLE_TO_ROTATE_LEFT)  ROTATE_LEFT( ROOT, W, OPT, HOOKS )
...
RECURSIVE FUNCTION LIST_NODE( ROOT, CURRENT, CNT, UNIT, PREFIX, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  IF ( .NOT.IS_LEAF ) THEN
    ...
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_LEFT_SUBTREE) LIST_NODE( ROOT, CURRENT%LEFT, CNT, UNIT, PREFIX, OPT, HOOKS )
    ...
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_RIGHT_SUBTREE) LIST_NODE( ROOT, CURRENT%RIGHT, CNT, UNIT, PREFIX,OPT,  HOOKS )
...
RECURSIVE FUNCTION FUNCTION_NODE( ROOT, CURRENT, PFUNCTION, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  IF ( .NOT.IS_LEAF ) THEN
    ...
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_LEFT_SUBTREE) FUNCTION_NODE( ROOT, CURRENT%LEFT, PFUNCTION, OPT, HOOKS )
    PP_TRYCALL(ERRFLAG_CALL_MAP_FUNCTION_ERROR) PFUNCTION( CURRENT%KEY, OPT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_RIGHT_SUBTREE) FUNCTION_NODE( ROOT, CURRENT%RIGHT, PFUNCTION, OPT, HOOKS )
...
RECURSIVE FUNCTION FUNCTOR_NODE( ROOT, CURRENT, FUNCTOR, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  IF ( .NOT.IS_LEAF ) THEN
    ...
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_LEFT_SUBTREE) FUNCTOR_NODE( ROOT, CURRENT%LEFT, FUNCTOR, OPT, HOOKS )
    PP_TRYCALL(ERRFLAG_CALL_MAP_FUNCTOR_ERROR) FUNCTOR%APPLY( CURRENT%KEY, OPT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_RIGHT_SUBTREE) FUNCTOR_NODE( ROOT, CURRENT%RIGHT, FUNCTOR, OPT, HOOKS )
...
RECURSIVE FUNCTION RENUMBER_NODE( ROOT, IDX, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_ISLEAF) NODE_ISLEAF( ROOT%LEFT, IS_LEAF, OPT, HOOKS )
  IF ( .NOT. IS_LEAF ) THEN
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_RENUMBER_NODE) RENUMBER_NODE( ROOT%LEFT, IDX, OPT, HOOKS )
  ENDIF

  ! Renumber right subtree
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_ISLEAF) NODE_ISLEAF( ROOT%RIGHT, IS_LEAF, OPT, HOOKS )
  IF ( .NOT. IS_LEAF ) THEN
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_RENUMBER_NODE) RENUMBER_NODE( ROOT%RIGHT, IDX, OPT, HOOKS )
  ENDIF
...
RECURSIVE FUNCTION NODE_WRITE_CONNECTIVITY( ROOT, UNIT, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_ISLEAF) NODE_ISLEAF( ROOT%LEFT, IS_LEAF, OPT, HOOKS )
  IF ( .NOT.IS_LEAF ) THEN
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_WRITE_CONNECTIVITY) NODE_WRITE_CONNECTIVITY( ROOT%LEFT, UNIT, OPT, HOOKS )
  ENDIF

  ! Write connectivity of the right subtree
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_ISLEAF) NODE_ISLEAF( ROOT%RIGHT, IS_LEAF, OPT, HOOKS )
  IF ( .NOT.IS_LEAF ) THEN
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_WRITE_CONNECTIVITY) NODE_WRITE_CONNECTIVITY( ROOT%RIGHT, UNIT, OPT, HOOKS )
  ENDIF

  ! Write connectivity of the current node
  IF ( ASSOCIATED(ROOT%PARENT) ) THEN
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_ISLEAF) NODE_ISLEAF( ROOT%PARENT, IS_LEAF, OPT, HOOKS )
    IF ( .NOT. IS_LEAF ) THEN
...
RECURSIVE FUNCTION WRITE_NODE( ROOT, UNIT, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  IF ( .NOT. ASSOCIATED( ROOT%LEFT, NIL ) ) THEN
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_WRITE_NODE) WRITE_NODE( ROOT%LEFT, UNIT,OPT,  HOOKS )
  ENDIF

  ! Write nodes in the right subtree
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_ISLEAF) NODE_ISLEAF( ROOT%RIGHT, IS_LEAF, OPT, HOOKS )
  IF ( .NOT. ASSOCIATED( ROOT%RIGHT, NIL ) ) THEN
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_WRITE_NODE) WRITE_NODE( ROOT%RIGHT, UNIT, OPT, HOOKS )
  ENDIF
...
RECURSIVE FUNCTION GET_SORTED_KEYS_NODE( NODE, SORTED_KEYS, CNT, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_ISLEAF) NODE_ISLEAF( NODE, IS_LEAF, OPT, HOOKS )
  IF ( .NOT. IS_LEAF ) THEN

    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_GET_SORTED_KEYS_NODE) GET_SORTED_KEYS_NODE( NODE%LEFT,  SORTED_KEYS, CNT, OPT, HOOKS )

    CNT = CNT + 1
    PP_DEBUG_CRITICAL_COND_THROW( CNT.GT.SIZE(SORTED_KEYS), ERRFLAG_OUT_OF_BOUNDS )
    SORTED_KEYS(CNT) = NODE%KEY

    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_GET_SORTED_KEYS_NODE) GET_SORTED_KEYS_NODE( NODE%RIGHT, SORTED_KEYS, CNT, OPT, HOOKS )
...
PP_THREAD_SAFE FUNCTION ENCODING_CACHE_INIT( ENCODERS_MAP, OPT, HOOKS ) RESULT(RET)
...
PP_THREAD_SAFE FUNCTION ENCODING_CACHE_FREE( ENCODERS_MAP, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  IF ( ENCODERS_MAP%SIZE .GT. 0 ) THEN
    ...
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_FREE) FREE_NODE( ENCODERS_MAP%ROOT, OPT, HOOKS )

  ENDIF

  ! Reset the initial condition.
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_INIT) ENCODERS_MAP%INIT( OPT, HOOKS )
...
PP_THREAD_SAFE FUNCTION ENCODING_CACHE_MINIMUM( ENCODERS_MAP, KEY, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  IF ( ENCODERS_MAP%SIZE .GT. 0 ) THEN
    ...
    ! Recursive deletion of the tree.
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_MINIMUM) MINIMUM( ENCODERS_MAP%ROOT, Y, OPT, HOOKS )
...
PP_THREAD_SAFE FUNCTION ENCODING_CACHE_MAXIMUM( ENCODERS_MAP, KEY, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  IF ( ENCODERS_MAP%SIZE .GT. 0 ) THEN
    ...
    ! Recursive deletion of the tree.
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_MINIMUM) MAXIMUM( ENCODERS_MAP%ROOT, Y, OPT, HOOKS )
...
PP_THREAD_SAFE FUNCTION ENCODING_CACHE_GET_SORTED_KEYS( ENCODERS_MAP, SORTED_KEYS, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  IF ( ENCODERS_MAP%SIZE .GT. 0 ) THEN
    ...
    ! Recursive deletion of the tree.
    CNT = 0
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_GET_SORTED_KEYS_NODE) GET_SORTED_KEYS_NODE( ENCODERS_MAP%ROOT, SORTED_KEYS, CNT, OPT, HOOKS )
  ENDIF
...
&  METADATA, ENCODING_RULES, ENCODERS, INSERTED, OPT, HOOKS )
...
PP_THREAD_SAFE FUNCTION ENCODING_CACHE_MATCH( THIS, KEY, OPT, LMATCH, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_ISLEAF) NODE_ISLEAF( THIS%ROOT, ROOT_IS_LEAF, OPT, HOOKS )
  IF ( ROOT_IS_LEAF ) THEN
  ...
  ELSE
    ...
    SEARCHED_NODE => NIL
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_SEARCH) SEARCH_NODE( THIS%ROOT, SEARCHED_NODE, KEY, OPT, LMATCH, HOOKS )
  ENDIF
...
PP_THREAD_SAFE FUNCTION ENCODING_CACHE_REMOVE( THIS, KEY, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  IF ( LMATCH ) THEN
    ...
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_REMOVE_NODE) REMOVE_NODE( THIS%ROOT, SEARCHED_NODE, OPT, HOOKS )
    THIS%SIZE = THIS%SIZE - 1
  ENDIF
...
PP_THREAD_SAFE FUNCTION ENCODING_CACHE_LIST( THIS, UNIT, PREFIX, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  CNT = 0_JPIB_K
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_LIST_NODE) LIST_NODE( THIS%ROOT, THIS%ROOT, CNT, UNIT, PREFIX, OPT, HOOKS )
...
PP_THREAD_SAFE FUNCTION ENCODING_CACHE_APPLY_FUNCTION( THIS, FUNCTION, OPT, HOOKS ) RESULT(RET)
...
PP_THREAD_SAFE FUNCTION ENCODING_CACHE_APPLY_FUNCTOR( THIS, FUNCTOR, OPT, HOOKS ) RESULT(RET)
...
PP_THREAD_SAFE FUNCTION ENCODING_CACHE_PRINT( THIS, NAME, IDX, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  UNIT = 0
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_RENUMBER_NODE) RENUMBER_NODE( THIS%ROOT, UNIT, OPT, HOOKS )
  UNIT=131
  ...
  IF ( .NOT. ROOT_IS_LEAF ) THEN
    ...
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STATUS .NE. 0, ERRFLAG_UNABLE_TO_WRITE_FILE )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_WRITE_NODE) WRITE_NODE( THIS%ROOT, UNIT, OPT, HOOKS )
    WRITE(UNIT,'(A)',IOSTAT=WRITE_STATUS) ''
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STATUS .NE. 0, ERRFLAG_UNABLE_TO_WRITE_FILE )
    ...
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_WRITE_CONNECTIVITY) NODE_WRITE_CONNECTIVITY( THIS%ROOT, UNIT, OPT, HOOKS )
    WRITE(UNIT,'(A)',IOSTAT=WRITE_STATUS) ''
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STATUS .NE. 0, ERRFLAG_UNABLE_TO_WRITE_FILE )
    WRITE(UNIT,'(A)',IOSTAT=WRITE_STATUS) '}'
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STATUS .NE. 0, ERRFLAG_UNABLE_TO_WRITE_FILE )
