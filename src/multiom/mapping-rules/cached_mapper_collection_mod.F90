...
&             IN_MSG, IN_PAR, OUT_MSG, OUT_PAR, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Perform the mapping
  PP_TRYCALL(ERRFLAG_MAPPING_RULES_EVAL) THIS%MAPPER_(IDX)%EVAL( IN_MSG, IN_PAR, OUT_MSG, OUT_PAR, HOOKS )
...
PP_THREAD_SAFE FUNCTION  CACHED_MAPPER_COLLECTION_FREE( THIS, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  IF ( ASSOCIATED(THIS%MAPPER_) ) THEN
    DO I = 1, SIZE(THIS%MAPPER_)
      PP_TRYCALL(ERRFLAG_FREE_CACHED_MAPPER) THIS%MAPPER_(I)%FREE( HOOKS )
    ENDDO
