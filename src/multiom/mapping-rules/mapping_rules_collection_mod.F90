...
PP_THREAD_SAFE FUNCTION RULE_MATCH( THIS, MSG, PAR, FILTER_OPT, MAPPING_INFO, HOOKS ) RESULT(RET)
  ...
  USE :: FILTER_OPTIONS_MOD,  ONLY: FILTER_OPTIONS_T
  USE :: MAPPING_INFO_MOD,    ONLY: MAPPING_INFO_T
...
IMPLICIT NONE
  ...
  TYPE(MAPPING_INFO_T), POINTER, DIMENSION(:), INTENT(OUT)   :: MAPPING_INFO
  TYPE(FILTER_OPTIONS_T), TARGET,              INTENT(IN)    :: FILTER_OPT
  TYPE(HOOKS_T),                               INTENT(INOUT) :: HOOKS
  ...
  IF ( NUM_MATCHES .GT. 0 ) THEN
    ...
    DO I = 1, NUM_MATCHES
      NULLIFY(ASSIGNMENT)
      TAG = REPEAT(' ',256)
      PP_TRYCALL(ERRFLAG_GET_ASSIGNMENTS) THIS%RULES_(THIS%MATCHES_(I))%RULE_%GET_ASSIGNMENT( ASSIGNMENT, TAG, HOOKS )
      PP_TRYCALL(ERRFLAG_INIT_MAPING_INFO) MAPPING_INFO(I)%INIT( ASSIGNMENT, TAG, HOOKS )
      THIS%MATCHES_(I) = -1_JPIB_K
    ENDDO
