...
PP_THREAD_SAFE FUNCTION DEBUG_POINT_COLLECTION_INIT( THIS, MAPPING_FNAME, HOOKS ) RESULT(RET)
  ...
  USE :: FORTRAN_MESSAGE_MOD, ONLY: FORTRAN_MESSAGE_READ_FROM_YAML
...
& TRIM(ADJUSTL(MAPPING_FNAME)), MAIN_TEST_CASES_CONFIG, HOOKS )
  ...
  IF ( HAS_TESTCASES  ) THEN
    ...
    DO I = 1, SZ
      ...
      PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_CONFIGURATION_HAS_KEY( TEST_CASES_CONFIGURATION, 'message', HAS_MSG, HOOKS )

        IF ( HAS_MSG ) THEN
            !> Read the message
            PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_GET_SUBCONFIGURATION( TEST_CASES_CONFIGURATION, 'message', MESSAGE_CONFIG, HOOKS )

            !> Read the message configuration
            PP_TRYCALL(ERRFLAG_READ_MESSAGE) FORTRAN_MESSAGE_READ_FROM_YAML( THIS%DEBUG_POINTS(I)%MSG, MESSAGE_CONFIG, HOOKS )

            !> Delete message configuration
            PP_TRYCALL(ERRFLAG_MAPPING_RULE_DELETE_ERROR) YAML_DELETE_CONFIGURATION( MESSAGE_CONFIG, HOOKS )
