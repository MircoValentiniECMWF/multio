...
PP_THREAD_SAFE FUNCTION MAKE_ENCODER_COLLECTION( MSG, PAR, &
        METADATA, ENCODING_RULES, OPT, ENCODERS_COLLECTION, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  IF ( IS_INITIALIZED ) THEN
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_RESET) ENCODERS_COLLECTION%FREE( OPT, HOOKS )
  ENDIF

  !> Get the encoders
  ENCODERS => NULL()
  PP_TRYCALL(ERRFLAG_UNABLE_TO_MATCH_RULE) ENCODING_RULES%MATCH( &
&  MSG, PAR, METADATA, ENCODERS, OPT, HOOKS )
