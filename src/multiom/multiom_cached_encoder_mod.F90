...
& CACHE_OPTIONS, ENCODER_OPTIONS, FILTER_OPTIONS, HOOKS ) RESULT(RET)
...
&     MAPPING_CONFIG, HOOKS )
  ...
  PP_TRYCALL(ERRFLAG_MAPPING_CACHE_INIT) THIS%MAPPING_CACHE%INIT( THIS%ENCODER_OPTIONS, HOOKS )
  PP_TRYCALL(ERRFLAG_ENCODER_CACHE_INIT) THIS%ENCODER_CACHE%INIT( THIS%ENCODER_OPTIONS, HOOKS )
...
IMPLICIT NONE
  ...
  & THIS%MAPPING_RULES, MAPPER, THIS%ENCODER_OPTIONS, THIS%FILTER_OPTIONS, HOOKS )
...
&                          MSG, PAR, MAPPED_MSG, MAPPED_PAR, HOOKS )
...
&       MAPPED_MSG, MAPPED_PAR, METADATA, ENCODING_DONE, &
&       THIS%ENCODER_OPTIONS, HOOKS )

      !> If encdong required/done then add the metadata the the list
      IF ( ENCODING_DONE ) THEN
        PP_TRYCALL(ERRFLAG_UNABLE_TO_ADD_METADATA) METADATA_LIST%PUSH( MAPPED_MSG, MAPPED_PAR, METADATA, HOOKS )
      ENDIF
...
IMPLICIT NONE
  ...
  PP_TRYCALL(ERRFLAG_ENCODER_RULES_FREE) THIS%ENCODER_CACHE%FREE( THIS%ENCODER_OPTIONS, HOOKS )

  !> Read the intop configuration
  PP_TRYCALL(ERRFLAG_MAPPING_RULES_FREE) THIS%MAPPING_RULES%FREE( HOOKS )
  PP_TRYCALL(ERRFLAG_MAPPING_CACHE_FREE) THIS%MAPPING_CACHE%FREE( THIS%ENCODER_OPTIONS, HOOKS )
