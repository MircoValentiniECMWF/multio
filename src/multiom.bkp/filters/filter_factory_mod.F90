...
RECURSIVE FUNCTION MAKE_FILTER( FILTER, CTYPE, CFG, HOOKS ) RESULT(RET)
  ...
  USE :: FILTER_LEVEL_MOD,               ONLY: FILTER_LEVEL_T
...
IMPLICIT NONE
  ...
  CASE( 'composed' )
    ...
    PP_TRYCALL(ERRFLAG_INITIALIZATION_ERROR)  FILTER%INIT( CFG, HOOKS )
    ...
    DO I = 1 , N_FILTERS
      ...
      PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_NESTED_FILTER) MAKE_FILTER( NESTED_FILTERS(I)%FILTER_, NESTED_CTYPE, NESTED_FILTER_CONFIGURATION, HOOKS )
  ...
  CASE( 'level' )
    ...
    ALLOCATE( FILTER_LEVEL_T::FILTER, STAT=ALLOC_STATUS, ERRMSG=ERRMSG )
    PP_DEBUG_CRITICAL_COND_THROW( ALLOC_STATUS.NE.0, ERRFLAG_ALLOCATION_ERROR )
