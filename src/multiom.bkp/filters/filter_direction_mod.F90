...
FUNCTION FILTER_DIRECTION_INIT( THIS, CFG, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  CASE( FLT_INT_MATCH_E )
    ...
    IF ( THIS%USE_KEYSET_ ) THEN
      PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_KEYSET_MATCH) THIS%DIRECTIONS_%MATCH( MSG%LEVEL(1), MATCH, HOOKS )
    ELSE
      MATCH = (THIS%DIRECTION_ .EQ. MSG%LEVEL(1))
    ENDIF
  ...
  CASE( FLT_INT_IGNORE_E )
    ...
    IF ( THIS%USE_KEYSET_ ) THEN
      ...
      MATCH = .NOT.MATCH
    ELSE
      MATCH = .NOT.(THIS%DIRECTION_ .EQ. MSG%LEVEL(1))
    ENDIF

  CASE( FLT_INT_GT_E )

    MATCH = (MSG%LEVEL(1) .GT. THIS%DIRECTION_ )

  CASE( FLT_INT_GE_E )

    MATCH = (MSG%LEVEL(1) .GE. THIS%DIRECTION_ )

  CASE( FLT_INT_LE_E )

    MATCH = (MSG%LEVEL(1) .LE. THIS%DIRECTION_ )

  CASE( FLT_INT_LT_E )

    MATCH = (MSG%LEVEL(1) .LT. THIS%DIRECTION_ )
...
IMPLICIT NONE
  ...
  WRITE(UNIT,*) REPEAT(' ',OFFSET)//'DIRECTION FILTER: '//TRIM(ADJUSTL(OPERATION_TYPE_STR))
