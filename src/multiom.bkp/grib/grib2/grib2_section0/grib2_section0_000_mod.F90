...
CONTAINS
  ...
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: PRINT => GRIB2_SECTION0_000_PRINT
...
!>
!> This function initializes a GRIB2 Section 0 object (`THIS`) using the provided model parameters (`PARAMS`)
...
!> @section interface
!>
!> @param [inout] THIS    GRIB2 Section 0 object to be initialized.
...
PP_THREAD_SAFE FUNCTION GRIB2_SECTION0_000_INIT( THIS, PAR, &
...
!> @section interface
!>
!> @param [in]    THIS      GRIB2 Section 0 object for which resources are allocated.
...
PP_THREAD_SAFE FUNCTION GRIB2_SECTION0_000_ALLOCATE( THIS, PAR, &
...
!> @section interface
!>
!> @param [in]    THIS      GRIB2 Section 0 object to be preset.
...
PP_THREAD_SAFE FUNCTION GRIB2_SECTION0_000_PRESET( THIS, PAR, &
&   MSG, METADATA, HOOKS ) RESULT(RET)
...
!> @section interface
!>
!> @param [in]    THIS          GRIB2 Section 0 object for runtime execution.
...
PP_THREAD_SAFE FUNCTION GRIB2_SECTION0_000_RUNTIME( THIS, PAR, &
...
!> @section interface
!>
!> @param [inout] THIS          GRIB2 Section 0 object to be checked for encoding readiness.
...
PP_THREAD_SAFE FUNCTION GRIB2_SECTION0_000_TO_BE_ENCODED( THIS, PAR, &
...
!> @section interface
!>
!> @param [inout] THIS    GRIB2 Section 0 object to be deallocated and freed.
...
PP_THREAD_SAFE FUNCTION GRIB2_SECTION0_000_FREE( THIS, HOOKS ) RESULT(RET)
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION0_000_PRINT'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION0_000_PRINT( THIS, UNIT, OFFSET, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  IF ( OFFSET .LE. 0 ) THEN
    WRITE(UNIT,'(A,A,A,A)', IOSTAT=WRITE_STAT) 'GRIB2_SECTION0_000_T: ', TRIM(ADJUSTL(THIS%NUMBER_)), '::', TRIM(ADJUSTL(THIS%TYPE_))
    PP_DEBUG_CRITICAL_COND_THROW(WRITE_STAT.NE.0, ERRFLAG_WRITE_ERROR )
  ELSE
    WRITE(UNIT,'(A,A,A,A,A)', IOSTAT=WRITE_STAT) REPEAT(' ',OFFSET), 'GRIB2_SECTION0_000_T: ', TRIM(ADJUSTL(THIS%NUMBER_)), '::', TRIM(ADJUSTL(THIS%TYPE_))
    PP_DEBUG_CRITICAL_COND_THROW(WRITE_STAT.NE.0, ERRFLAG_WRITE_ERROR )
  ENDIF
...
END FUNCTION GRIB2_SECTION0_000_PRINT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
