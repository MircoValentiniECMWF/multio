...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERR_FLAG_READ_ENCODER_TYPE = 3_JPIB_K
  ...
  PP_TRYCALL(ERR_FLAG_READ_ENCODER_TYPE) READ_ENCODER_TYPE( CONFIG, ENCODER_TYPE, HOOKS )

  !> Create the encoder object
  PP_TRYCALL(ERR_FLAG_MAKE_ENCODER) MAKE_ENCODER( ENCODER, ENCODER_TYPE, PAR, CONFIG, OPT, HOOKS )

  !> Print the encoder object
   PP_TRYCALL(ERR_FLAG_PRINT_ENCODER) ENCODER%PRINT( UNIT, OFFSET, HOOKS )

  !> Destroy the encoder object
  PP_TRYCALL(ERR_FLAG_DESTROY_ENCODER) DESTROY_ENCODER( ENCODER, HOOKS )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    SELECT CASE(ERRIDX)
      ...
      CASE(ERR_FLAG_READ_ENCODER_TYPE)
        PP_DEBUG_PUSH_MSG_TO_FRAME( 'Error reading the encoder type' )
