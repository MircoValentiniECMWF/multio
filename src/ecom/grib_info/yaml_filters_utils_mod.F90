...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_PARAMID_ALLOCATED)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'FILTER%PARAMID already allocated allocated' )
    CASE (ERRFLAG_LEVEL_ALLOCATED)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'FILTER%LEVEL already allocated allocated' )
    CASE (ERRFLAG_DIRECTION_ALLOCATED)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'FILTER%DIRECTION already allocated allocated' )
    CASE (ERRFLAG_FREQUENCY_ALLOCATED)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'FILTER%FREQUENCY already allocated allocated' )
    CASE (ERRFLAG_REPRES_ALLOCATED)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'FILTER%REPRES already allocated allocated' )
    CASE (ERRFLAG_LEVTYPE_ALLOCATED)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'FILTER%LEVTYPE already allocated allocated' )
    CASE (ERRFLAG_TAG_ALLOCATED)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'FILTER%TAG already allocated allocated' )
    CASE (ERRFLAG_NESTED_READ_PARAMID)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'Nested error calling: READ_RULE_FILTER_PARAMID' )
    CASE (ERRFLAG_NESTED_READ_LEVEL)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'Nested error calling: READ_RULE_FILTER_LEVEL' )
    CASE (ERRFLAG_NESTED_READ_DIRECTION)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'Nested error calling: READ_RULE_FILTER_DIRECTION' )
    CASE (ERRFLAG_NESTED_READ_FREQUENCY)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'Nested error calling: READ_RULE_FILTER_FREQUENCY' )
    CASE (ERRFLAG_NESTED_READ_REPRES)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'Nested error calling: READ_RULE_FILTER_REPRES' )
    CASE (ERRFLAG_NESTED_READ_LEVTYPE)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'Nested error calling: READ_RULE_FILTER_LEVTYPE' )
    CASE (ERRFLAG_NESTED_READ_TAG)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'Nested error calling: READ_RULE_FILTER_TAG' )
    CASE (ERRFLAG_NESTED_READ_ISENSEMBLE)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'Nested error calling: READ_RULE_FILTER_ISENSEMBLE' )
    CASE (ERRFLAG_NESTED_READ_ISCHEMICAL)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'Nested error calling: READ_RULE_FILTER_ISCHEMICAL' )
    CASE DEFAULT
  ...
  BLOCK
    ...
    CASE (ERRFLAG_DEALLOC_PARAMID)
      IF ( .NOT.ALLOCATED(ERRMSG) ) THEN
        PP_DEBUG_CREATE_ERROR_MSG( STR, 'error deallocating PARAMID' )
      ELSE
        PP_DEBUG_CREATE_ERROR_MSG( STR, 'error deallocating PARAMID: '//TRIM(ADJUSTL(ERRMSG)) )
        DEALLOCATE(ERRMSG)
      ENDIF
    CASE (ERRFLAG_DEALLOC_LEVEL)
      IF ( .NOT.ALLOCATED(ERRMSG) ) THEN
        PP_DEBUG_CREATE_ERROR_MSG( STR, 'error deallocating LEVEL' )
      ELSE
        PP_DEBUG_CREATE_ERROR_MSG( STR, 'error deallocating LEVEL: '//TRIM(ADJUSTL(ERRMSG)) )
        DEALLOCATE(ERRMSG)
      ENDIF
    CASE (ERRFLAG_DEALLOC_DIRECTION)
      IF ( .NOT.ALLOCATED(ERRMSG) ) THEN
        PP_DEBUG_CREATE_ERROR_MSG( STR, 'error deallocating DIRECTION' )
      ELSE
        PP_DEBUG_CREATE_ERROR_MSG( STR, 'error deallocating DIRECTION: '//TRIM(ADJUSTL(ERRMSG)) )
        DEALLOCATE(ERRMSG)
      ENDIF
    CASE (ERRFLAG_DEALLOC_FREQUENCY)
      IF ( .NOT.ALLOCATED(ERRMSG) ) THEN
        PP_DEBUG_CREATE_ERROR_MSG( STR, 'error deallocating FREQUENCY' )
      ELSE
        PP_DEBUG_CREATE_ERROR_MSG( STR, 'error deallocating FREQUENCY: '//TRIM(ADJUSTL(ERRMSG)) )
        DEALLOCATE(ERRMSG)
      ENDIF
    CASE (ERRFLAG_DEALLOC_REPRES)
      IF ( .NOT.ALLOCATED(ERRMSG) ) THEN
        PP_DEBUG_CREATE_ERROR_MSG( STR, 'error deallocating REPRES' )
      ELSE
        PP_DEBUG_CREATE_ERROR_MSG( STR, 'error deallocating REPRES: '//TRIM(ADJUSTL(ERRMSG)) )
        DEALLOCATE(ERRMSG)
      ENDIF
    CASE (ERRFLAG_DEALLOC_LEVTYPE)
      IF ( .NOT.ALLOCATED(ERRMSG) ) THEN
        PP_DEBUG_CREATE_ERROR_MSG( STR, 'error deallocating LEVTYPE' )
      ELSE
        PP_DEBUG_CREATE_ERROR_MSG( STR, 'error deallocating LEVTYPE: '//TRIM(ADJUSTL(ERRMSG)) )
        DEALLOCATE(ERRMSG)
      ENDIF
    CASE (ERRFLAG_DEALLOC_TAG)
      IF ( .NOT.ALLOCATED(ERRMSG) ) THEN
        PP_DEBUG_CREATE_ERROR_MSG( STR, 'error deallocating TAG' )
      ELSE
        PP_DEBUG_CREATE_ERROR_MSG( STR, 'error deallocating TAG: '//TRIM(ADJUSTL(ERRMSG)) )
        DEALLOCATE(ERRMSG)
      ENDIF
    CASE DEFAULT
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'unhandled error' )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_PARAMID_NOT_ALLOCATED)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'FILTER_PARAMID not allocated' )
    CASE (ERRFLAG_PARAMID_EMPTY)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'FILTER_PARAMID not allocated or empty' )
    CASE (ERRFLAG_KEY_NOT_PRESENT)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'Nested error calling: YAML_CONFIGURATION_HAS_KEY' )
    CASE (ERRFLAG_READ_KEY_FAILED)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'Nested error calling: YAML_READ_INTEGER_ARRAY_WITH_RANGES' )
    CASE DEFAULT
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'unhandled error' )
    END SELECT
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_LEVEL_NOT_ALLOCATED)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'FILTER_LEVEL not allocated' )
    CASE (ERRFLAG_LEVEL_EMPTY)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'FILTER_LEVEL not allocated or empty' )
    CASE (ERRFLAG_KEY_NOT_PRESENT)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'Nested error calling: YAML_CONFIGURATION_HAS_KEY' )
    CASE (ERRFLAG_READ_KEY_FAILED)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'Nested error calling: YAML_READ_INTEGER_ARRAY_WITH_RANGES' )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_DIRECTION_NOT_ALLOCATED)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'FILTER_DIRECTION not allocated' )
    CASE (ERRFLAG_DIRECTION_EMPTY)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'FILTER_DIRECTION not allocated or empty' )
    CASE (ERRFLAG_KEY_NOT_PRESENT)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'Nested error calling: YAML_CONFIGURATION_HAS_KEY' )
    CASE (ERRFLAG_READ_KEY_FAILED)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'Nested error calling: YAML_READ_INTEGER_ARRAY_WITH_RANGES' )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_FREQUENCY_NOT_ALLOCATED)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'FILTER_FREQUENCY not allocated' )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_REPRES_NOT_ALLOCATED)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'FILTER_REPRES not allocated' )
    CASE (ERRFLAG_REPRES_EMPTY)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'FILTER_REPRES not allocated or empty' )
    CASE (ERRFLAG_KEY_NOT_PRESENT)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'Nested error calling: YAML_CONFIGURATION_HAS_KEY' )
    CASE (ERRFLAG_READ_KEY_FAILED)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'Nested error calling: YAML_READ_INTEGER_ARRAY_WITH_RANGES' )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_LEVTYPE_NOT_ALLOCATED)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'FILTER_LEVTYPE not allocated' )
    CASE (ERRFLAG_LEVTYPE_EMPTY)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'FILTER_LEVTYPE not allocated or empty' )
    CASE (ERRFLAG_KEY_NOT_PRESENT)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'Nested error calling: YAML_CONFIGURATION_HAS_KEY' )
    CASE (ERRFLAG_READ_KEY_FAILED)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'Nested error calling: YAML_READ_INTEGER_ARRAY_WITH_RANGES' )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_TAG_NOT_ALLOCATED)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'FILTER_TAG not allocated' )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_ISENSEMBLE_NOT_ALLOCATED)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'FILTER_ISENSEMBLE not allocated' )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_ISCHEMICAL_NOT_ALLOCATED)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'FILTER_ISCHEMICAL not allocated' )
    CASE (ERRFLAG_ISCHEMICAL_EMPTY)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'FILTER_ISCHEMICAL not allocated or empty' )
    CASE (ERRFLAG_KEY_NOT_PRESENT)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'Nested error calling: YAML_CONFIGURATION_HAS_KEY' )
    CASE (ERRFLAG_READ_KEY_FAILED)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'Nested error calling: YAML_READ_INTEGER_ARRAY_WITH_RANGES' )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_EMPTY_FILTER)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'EMPTY FILTERS are not allowed' )
    CASE DEFAULT
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'unhandled error' )
    END SELECT
