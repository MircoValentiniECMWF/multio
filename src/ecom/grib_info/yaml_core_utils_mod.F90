...
MODULE YAML_CORE_UTILS_MOD
  ...
  PP_USE_E('P') :: FCKIT, ONLY: FCKIT_CONFIGURATION
...
TYPE :: YAML_CONFIGURATIONS_T
  ...
  TYPE(FCKIT_CONFIGURATION), DIMENSION(:), POINTER :: CFGS_ => NULL()
ENDTYPE
...
INTERFACE
  FUNCTION FUN_C2I_IF( CHAR, I ) RESULT( RET )
...
!> which is then returned in the `VALUE` argument. The process can be controlled by the `VERBOSE` flag,
!> which enables additional output for debugging purposes.
!>
!> @section interface
...
!> @param [in] VERBOSE Logical flag to enable verbose output (`.TRUE.`) for debugging purposes.
!>
...
__THREAD_SAFE__ FUNCTION YAML_NEW_CONFIGURATION_FROM_FILE( YAML_FILE_NAME, VALUE, VERBOSE )

  ! Symbols imported from other modules within the project.
  PP_USE_L('P') :: OM_CORE_MOD, ONLY: JPIB_K

  ! Symbols imported from other libraries
  PP_USE_E('S')  :: FCKIT_PATHNAME_MODULE,      ONLY: FCKIT_PATHNAME
  PP_USE_E('S')  :: FCKIT_CONFIGURATION_MODULE, ONLY: FCKIT_YAMLCONFIGURATION
...
IMPLICIT NONE
  ...
  ! Local error codes
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_CFG_ALREADY_ALLOCATED=1_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_CFG_ALLOCATED_FLAG=2_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_YAML_FILE_NOT_PRESENT=3_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_CFG_ALLOCATE_ERROR=4_ERR_K
  ...
  ! Erro handling
  PP_DEBUG_CRITICAL_COND_THROW( ALLOCATED(VALUE%CFG_), 1 )
  PP_DEBUG_CRITICAL_COND_THROW( VALUE%IS_ALLOCATED_, 2 )

  ! Check if the file exsts
  INQUIRE( FILE=TRIM(YAML_FILE_NAME), EXIST=EX )
...
!>
!> This function removes a specific configuration from the provided YAML configuration object (`CFG`).
!> The removal process can be controlled by the `VERBOSE` flag, which enables additional output for
!> debugging purposes. If the configuration is allocated, the it will be deallocated, while if the
!> configuration is just a pointer to another configuration, then the pointer will be set to `NULL`.
!>
!> @section interface
...
__THREAD_SAFE__ FUNCTION YAML_DELETE_CONFIGURATION( CFG, VERBOSE ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Local error codes
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_DEALLOC_CONFIGURATION=1_ERR_K
...
!>
!> This function removes configurations from the provided YAML configurations object (`CFG`).
!> The operation can be controlled by the `VERBOSE` flag, which enables additional output for
!> debugging purposes.
!>
!> @section interface
...
__THREAD_SAFE__ FUNCTION YAML_DELETE_CONFIGURATIONS( CFG, VERBOSE ) RESULT(RET)

  ! Symbols imported from other modules within the project.
  PP_USE_L('P') :: OM_CORE_MOD, ONLY: JPIB_K

  ! Symbols imported from other libraries
  PP_USE_E('S') :: FCKIT_CONFIGURATION_MODULE, ONLY: DEALLOCATE_FCKIT_CONFIGURATION
...
IMPLICIT NONE
  ...
  ! Local error codes
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_DEALLOC_CONFIGURATIONS=1_ERR_K
...
!> based on the given `KEY`. The extracted sub-configuration is stored in `VALUE`. The function supports
!> verbose output controlled by the `VERBOSE` flag for debugging purposes.
!>
!> @section interface
...
__THREAD_SAFE__ FUNCTION YAML_GET_SUBCONFIGURATION( CFG, KEY, VALUE, VERBOSE ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Local error codes
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_CFG_NOT_ALLOCATED=1_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_STRING_ARRAY_ALSREADY_ALLOCATED=2_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_UNABLE_TO_SUBCCONFIGURATION=3_ERR_K
...
!> Verbose output can be enabled via the `VERBOSE` flag for debugging purposes.
!>
!> @section interface
...
__THREAD_SAFE__ FUNCTION YAML_GET_SUBCONFIGURATIONS( CFG, KEY, VALUE, VERBOSE ) RESULT(RET)
...
IMPLICIT NONE
  ...
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_CFG_NOT_ALLOCATED=1_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_STRING_ARRAY_ALSREADY_ALLOCATED=2_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_UNABLE_TO_SUBCCONFIGURATIONS=3_ERR_K
...
!> and returns the result in `CFG_SIZE`. It also supports verbose mode for debugging
!> purposes, providing additional output if needed.
!>
!> @section interface
...
__THREAD_SAFE__ FUNCTION YAML_GET_CONFIGURATIONS_SIZE( CFG, CFG_SIZE, VERBOSE ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Error handling
  PP_DEBUG_CRITICAL_COND_THROW( .NOT.ASSOCIATED(CFG%CFGS_), ERRFLAG_CFGS_NOT_ALLOCATED )

  ! Get the configuration size
  CFG_SIZE = SIZE(GFG%CFGS_)
...
!> in `CURR_CFG`. Verbose mode can be enabled to display additional debug information during execution.
!>
!> @section interface
...
!> @param [in] VERBOSE Logical flag for enabling verbose output (`.TRUE.`) for debugging purposes.
!>
...
__THREAD_SAFE__ FUNCTION YAML_GET_CONFIGURATION_BY_ID( CFG, CFG_ID, CURR_CFG, VERBOSE ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(YAML_CONFIGURATIONS_T), INTENT(INOUT) :: CFG
  INTEGER(KIND=JPIB_K),        INTENT(OUT)   :: CFG_ID
  ...
  ! Local error codes
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_CFG_NOT_ALLOCATED=1_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_OUT_OF_BOUNDS_UB=2_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_OUT_OF_BOUNDS_LB=3_ERR_K
...
!> It returns a logical flag (`HAS_KEY`) indicating whether the key was found. If verbose mode
!> is enabled, additional information is printed during execution.
!>
!> @section interface
...
__THREAD_SAFE__ FUNCTION YAML_CONFIGURATION_HAS_KEY( CFG, KEY, HAS_KEY, VERBOSE ) RESULT(EXISTS)
...
!> If an error occurs during the process, the function returns an error code. Verbose mode can be
!> enabled to display additional debug information.
!>
!> @section interface
...
__THREAD_SAFE__ FUNCTION YAML_READ_STRING_ARRAY( CFG, KEY, VALUE, VERBOSE ) RESULT(RET)
...
IMPLICIT NONE
  ...
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_CFG_NOT_ALLOCATED=1_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_STRING_ARRAY_ALSREADY_ALLOCATED=2_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_UNABLE_TO_READ_STRING_ARRAY=3_ERR_K
...
!> indicating success or failure. If verbose mode is enabled, additional debug information is printed.
!>
!> @section interface
...
__THREAD_SAFE__ FUNCTION YAML_READ_INTEGER_ARRAY_WITH_FILTER( CFG, KEY, VALUE, FILTER, VERBOSE ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Local error codes
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_CFG_NOT_ALLOCATED=1_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_VALUE_ALREADY_ALLOCATED=2_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_STRING_ARRAY_NOT_ALLOCATED=3_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_STRING_ARRAY_SIZE_LT_1=4_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_ALLOCATION_ERROR=5_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_CALL_FILTER=6_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_DEALLOCATION_ERROR=7_ERR_K
  ...
  CALL YAML_READ_STRING_ARRAY( CFG, KEY, ATMP, VERBOSE )
  ...
  ParamIdFIlterSizeLoop: DO I = 1, VALUE_SIZE
    ...
    ! Filter the value
    PP_TRYCALL(ERRFLAG_CALL_FILTER) FILTER(ATMP(I), VALUE(I))
...
__THREAD_SAFE__ FUNCTION YAML_READ_INTEGER_ARRAY_WITH_RANGES( CFG, KEY, VALUE, VERBOSE ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Local error codes
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_CFG_NOT_ALLOCATED=1_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_STRING_ARRAY_NOT_ALLOCATED=2_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_STRING_ARRAY_SIZE_LT_1=3_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_INVALID_STRING_IN_ARRAY_1=4_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_INVALID_STRING_IN_ARRAY_2=5_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_INVALID_STRING_IN_ARRAY_3=6_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_INVALID_STRING_IN_ARRAY_4=7_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_INVALID_STRING_IN_ARRAY_5=8_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_ALLOCATION_ERROR=10_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_VALUE_OUT_OF_BOUNDS=12_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_DEALLOCATION_ERROR=13_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_VALUS_SIZE_LT_1=14_ERR_K
  ...
  ParamIdFIlterSizeLoop: DO I = 1, SIZE(ATMP)
    ...
    CALL STRING_IS_INTEGER         ( ATMP(I), IS_INTEGER,          VERBOSE )
    CALL STRING_IS_INTEGER_RANGE   ( ATMP(I), IS_INTEGER_RANGE,    VERBOSE )
    CALL STRING_IS_INTEGER_RANGE_BY( ATMP(I), IS_INTEGER_RANGE_BY, VERBOSE )
    ...
    ELSE IF ( IS_INTEGER_RANGE ) THEN
      CALL STRING_TO_INTEGER_RANGE( ATMP(I), LO, HI, VERBOSE )
      VALUE_SIZE = VALUE_SIZE + HI - LO + 1
    ELSE IF ( IS_INTEGER_RANGE_BY ) THEN
      CALL STRING_TO_INTEGER_RANGE_BY( ATMP(I), LO, HI, BY, VERBOSE )
  ...
  ParamIdFIlterFillLoop: DO I = 1, SIZE(ATMP)
    ...
    IF ( IS_INTEGER ) THEN
      ...
      CALL STRING_TO_INTEGER( ATMP(I), VALUE(J), VERBOSE )
...
!> This function reads an integer array from a provided YAML configuration object (`CFG`)
!> using the specified key (`KEY`). The `VALUE` array is populated with the parsed integers.
!> If an error occurs during the reading process, the function returns an error code. Verbose mode
!> can be enabled for additional debugging information.
!>
!> @section interface
!> @param [in] CFG The YAML configuration object from which the integer array is read.
!> @param [in] KEY The key corresponding to the integer array in the YAML configuration.
...
__THREAD_SAFE__ FUNCTION YAML_READ_INTEGER_ARRAY( CFG, KEY, VALUE, VERBOSE ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Local error codes
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_CFG_NOT_ALLOCATED=1_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_UNABLE_TO_READ_INTEGER_ARRAY=2_ERR_K
...
!> It supports a verbose mode for additional output during the operation. If an error occurs,
!> an error code is returned to indicate the issue.
!>
!> @section interface
!>
...
__THREAD_SAFE__ FUNCTION YAML_READ_STRING( CFG, KEY, VALUE, VERBOSE ) RESULT(RET)
...
IMPLICIT NONE
  ...
  TYPE(YAML_CONFIGURATION_T),    INTENT(IN)  :: CFG
  CHARACTER(LEN=*),              INTENT(IN)  :: KEY
  CHARACTER(LNE=:), ALLOCATABLE, INTENT(OUT) :: VALUE
  LOGICAL,                       INTENT(IN)  :: VERBOSE
  ...
  ! Local error codes
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_CFG_NOT_ALLOCATED=1_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_UNABLE_TO_READ_STRING=2_ERR_K
...
__THREAD_SAFE__ FUNCTION YAML_READ_INTEGER( CFG, KEY, VALUE, VERBOSE ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Local error codes
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_CFG_NOT_ALLOCATED=1_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_UNABLE_TO_READ_INTEGER=2_ERR_K
...
!> Verbose mode can be enabled for detailed output during the operation.
!>
!> @section interface
!>
...
!>
!> @subsection module dependencies
!>   - @dependency [PARAMETER] OM_CORE_MOD::JPRD_K
!>
!> @subsection special dependencies
!>   - @dependency [*] PP_DEBUG_USE_VARS::*
!>   - @dependency [*] PP_LOG_USE_VARS::*
...
__THREAD_SAFE__ FUNCTION YAML_READ_FLOAT( CFG, KEY, VALUE, VERBOSE ) RESULT(RET)

  ! Symbols imported from other modules within the project.
  PP_USE_L('P') :: OM_CORE_MOD, ONLY: JPRD_K
...
IMPLICIT NONE
  ...
  ! Local error codes
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_CFG_NOT_ALLOCATED=1_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_UNABLE_TO_READ_FLOAT=2_ERR_K
...
!> If an error occurs, it returns an appropriate error code. Verbose mode can be
!> enabled for detailed output during the process.
!>
!> @section interface
!>
...
__THREAD_SAFE__ FUNCTION YAML_READ_LOGICAL( CFG, VALUE, VERBOSE ) RETURN(RET)
...
IMPLICIT NONE
  ...
  ! Local error codes
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_CFG_NOT_ALLOCATED=1_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_UNABLE_TO_READ_LOGICAL=2_ERR_K
...
!>
!> The function also supports a `VERBOSE` mode
!> for additional output during debugging or verbose operation.
!>
!> @section interface
!> @param [in] STRING   Input string representing the range (e.g. "1:10").
!> @param [out] MATCH   True if the string has the format "LO:HI".
!> @param [in] VERBOSE  Logical flag indicating whether verbose output is enabled.
!>
...
__THREAD_SAFE__ FUNCTION STRING_IS_INTEGER( STRING, MATCH, VERBOSE ) RETURN(RET)
...
IMPLICIT NONE
  ...
  ! Local error codes
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_PARSE_ERROR=1_ERR_K
  ...
  ! Parse the string
  PP_TRYCALL(ERRFLAG_PARSE_ERROR) READ_INTEGER_PATTERNS( STRING, LTMP, COUNT, VERBOSE, VALUES=TMP )
...
__THREAD_SAFE__ FUNCTION STRING_IS_INTEGER_RANGE( STRING, MATCH, VERBOSE ) RETURN(RET)
...
__THREAD_SAFE__ FUNCTION STRING_IS_INTEGER_RANGE_BY( STRING, MATCH, VERBOSE )
...
!> If the conversion fails, the function returns an error code. It also supports a
!> `VERBOSE` mode, which provides additional output for debugging purposes.
!>
!> @section interface
...
__THREAD_SAFE__ FUNCTION STRING_TO_INTEGER( STRING, VALUE, VERBOSE ) RETURN(RET)
...
IMPLICIT NONE
  ...
  ! Local error codes
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_PARSE_ERROR=1_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_NO_MATCH=2_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_WRONG_COUNT=3_ERR_K
  ...
  ! Parse the string
  PP_TRYCALL(ERRFLAG_PARSE_ERROR) READ_INTEGER_PATTERNS( STRING, MATCH, COUNT, VERBOSE, VALUES=TMP )
...
!> converts it into integers `LO` and `HI`. The function also supports a `VERBOSE` mode
!> for additional output during debugging or verbose operation.
!>
!> @section interface
!> @param [in] STRING   Input string representing the range (e.g., "1:10").
...
__THREAD_SAFE__ FUNCTION STRING_TO_INTEGER_RANGE( STRING, LO, HI, VERBOSE ) RETURN(RET)
...
IMPLICIT NONE
  ...
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_NO_MATCH=2_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_WRONG_COUNT=3_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_WRONG_RANGE=4_ERR_K
...
!> converts it into integers `LO`, `HI`, and `BY`. The function also supports a `VERBOSE` mode
!> for additional output during debugging or verbose operation.
!>
!> @section interface
!> @param [in] STRING   Input string representing the range (e.g. "1:10:2" ).
...
__THREAD_SAFE__ FUNCTION STRING_TO_INTEGER_RANGE_BY( STRING, LO, HI, BY, VERBOSE ) RESULT(RET)
...
IMPLICIT NONE
  ...
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_WRONG_COUNT=3_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_WRONG_RANGE=4_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_STEP_SIGN=5_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_STEP_SIZE=6_ERR_K
...
__THREAD_SAFE__ FUNCTION  READ_INTEGER_PATTERNS( STRING, MATCH, COUNT, VERBOSE, VALUES ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Error Codes
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_OK=0_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_INVALID_CHAR_IN_CHECK_FIRST_NUM=1_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_INVALID_CHAR_IN_SEARCH_SEPARATOR=2_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_INVALID_CHAR_IN_SEPARATOR=3_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_OOB_IN_OUT_VALUES=4_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_UNEXPECTED=5_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_FAILED_READ_INTEGER=6_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_GLB_IDX_LOWER_THAN_1=7_ERR_K
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_GLB_IDX_BIGGER_THAN_STRING=8_ERR_K
  ...
  StatesLoop: DO WHILE (LOOP)
    ...
    CASE (VALID_VALUE_STATE)
      ...
      IF ( PRESENT(VALUES) ) THEN
        IF ( READ_INTEGER( STRING(LO:HI), TMP, VERBOSE ) .NE. 0 ) THEN
...
__THREAD_SAFE__ FUNCTION  READ_INTEGER( S, NUM, VERBOSE ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Local error codes
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_INVALID_STRING=1_ERR_K
