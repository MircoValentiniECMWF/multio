...
!> to the `GRIB_SECTION5` object and configures it using the provided model parameters,
!> ID, and YAML configuration. If verbose mode is enabled, additional debug information
!> is output during the process.
!>
!> @param [inout] GRIB_SECTION5 The GRIB2 Section 5 object that will be created or initialized.
...
FUNCTION GRIB2_SECTION5_FACTORY( GRIB_SECTION5, PARAMS, ID, CFG, VERBOSE ) RESULT(RET)

  !> Symbols imported from other modules within the project.
  PP_USE_L('P') :: OM_CORE_MOD,            ONLY: JPIB_K
  PP_USE_L('T') :: GRIB2_SECTION5_001_MOD, ONLY: GRIB2_SECTION5_001_T
  PP_USE_L('T') :: GRIB2_SECTION5_042_MOD, ONLY: GRIB2_SECTION5_042_T
  PP_USE_L('T') :: GRIB2_SECTION5_051_MOD, ONLY: GRIB2_SECTION5_051_T
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  CLASS(GRIB_SECTION_BASE_A), POINTER, INTENT(INOUT) :: GRIB_SECTION5
  ...
  CHARACTER(LEN=:), ALLOCATABLE :: ERRMSG
   ...
   ! Local error codes
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_UNKNOWN_SECTION_5=1_ERR_K
  ...
  CASE( 1 )
    ...
    ALLOCATE( GRIB2_SECTION5_001_T::GRIB_SECTION5, STAT=ALLOC_STATUS, ERRMSG=ERRMSG )
    PP_DEBUG_CRITICAL_COND_THROW( ALLOC_STATUS.NE.0, ERRFLAG_ALLOCATION_ERROR )

  CASE( 42 )

    ALLOCATE( GRIB2_SECTION5_042_T::GRIB_SECTION5, STAT=ALLOC_STATUS, ERRMSG=ERRMSG )
    PP_DEBUG_CRITICAL_COND_THROW( ALLOC_STATUS.NE.0, ERRFLAG_ALLOCATION_ERROR )

  CASE( 51 )

    ALLOCATE( GRIB2_SECTION5_051_T::GRIB_SECTION5, STAT=ALLOC_STATUS, ERRMSG=ERRMSG )
    PP_DEBUG_CRITICAL_COND_THROW( ALLOC_STATUS.NE.0, ERRFLAG_ALLOCATION_ERROR )
  ...
  PP_TRYCALL(ERRFLAG_INITIALIZATION_ERROR)  GRIB2_SECTION5%INIT( PARAMS, CFG, VERBOSE )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_UNKNOWN_SECTION_5)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'Unknown section1 number: '//TRIM(ADJUSTL(TMP)) )
    CASE (ERRFLAG_ALLOCATION_ERROR)
      IF ( .NOT.ALLOCATED(ERRMSG) ) THEN
        PP_DEBUG_CREATE_ERROR_MSG( STR, 'error allocating section1 number: '//TRIM(ADJUSTL(TMP)) )
      ELSE
        PP_DEBUG_CREATE_ERROR_MSG( STR, 'error allocating section1 number: '//TRIM(ADJUSTL(TMP))//' : '//TRIM(ADJUSTL(ERRMSG)) )
        DEALLOCATE(ERRMSG)
      ENDIF
    CASE (ERRFLAG_INITIALIZATION_ERROR)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'error initializing section1 number: '//TRIM(ADJUSTL(TMP)) )
    CASE DEFAULT
...
END FUNCTION GRIB2_SECTION5_FACTORY
