...
!> to the `GRIB_SECTION1` object and configures it using the provided model parameters,
!> ID, and YAML configuration. If verbose mode is enabled, additional debug information
!> is output during the process.
!>
!> @param [inout] GRIB_SECTION1 The GRIB2 Section 1 object that will be created or initialized.
!>                              It must be a pointer of type `GRIB_SECTION_BASE_A`.
!> @param [in] PARAMS The model parameters structure of type `MODEL_PAR_T`.
!> @param [in] ID Integer identifier for the GRIB2 Section 1 object.
...
#define PP_PROCEDURE_NAME 'GRIB2_SECTION1_FACTORY'
FUNCTION GRIB2_SECTION1_FACTORY( GRIB_SECTION1, PARAMS, ID, CFG, VERBOSE ) RESULT(RET)

  !> Symbols imported from other modules within the project.
  PP_USE_L('P') :: OM_CORE_MOD,            ONLY: JPIB_K
  PP_USE_L('T') :: GRIB2_SECTION1_000_MOD, ONLY: GRIB2_SECTION1_000_T
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  CLASS(GRIB_SECTION_BASE_A), POINTER, INTENT(INOUT) :: GRIB_SECTION1
  ...
  CHARACTER(LEN=:), ALLOCATABLE :: ERRMSG
   ...
   ! Local error codes
  INTEGER(KIND=ERR_K), PARAMETER :: ERRFLAG_UNKNOWN_SECTION_1=1_ERR_K
  ...
  CASE( 0 )
    ...
    ALLOCATE( GRIB2_SECTION1_000_T::GRIB_SECTION1, STAT=ALLOC_STATUS, ERRMSG=ERRMSG )
  ...
  PP_TRYCALL(ERRFLAG_INITIALIZATION_ERROR)  GRIB2_SECTION1%INIT( PARAMS, CFG, VERBOSE )
...
END FUNCTION GRIB2_SECTION1_FACTORY
