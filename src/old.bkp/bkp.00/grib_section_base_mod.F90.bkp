...
CONTAINS
  ...
  PROCEDURE(GRIB_SECTION_INIT_IF), DEFERRED, PASS, PUBLIC :: INIT
...
!>   @param [in] PAR The parametrization structure of type `PARAMETRIZATION_T`.
...
!>
PP_THREAD_SAFE FUNCTION GRIB_SECTION_INIT_IF( THIS, PAR, &
...
!>   @param [in] PAR The parametrization structure of type `PARAMETRIZATION_T` used for allocation.
!>   @param [in] MSG The message object of type `FORTRAN_MESSAGE_T` for allocation-related messaging.
...
!>
PP_THREAD_SAFE FUNCTION GRIB_SECTION_ALLOCATE_IF( THIS, PAR, &
...
!>
PP_THREAD_SAFE FUNCTION GRIB_SECTION_PRESET_IF( THIS, PAR, &
&   MSG, METADATA, HOOKS ) RESULT(RET)
...
!>   @param [in] PAR The parametrization structure of type `PARAMETRIZATION_T` used during the runtime phase.
!>   @param [in] MSG The message object of type `FORTRAN_MESSAGE_T` for runtime-related messaging.
...
!>
PP_THREAD_SAFE FUNCTION GRIB_SECTION_RUNTIME_IF( THIS, PAR, &
...
!>   @param [in] PAR The parametrization structure of type `PARAMETRIZATION_T` used to influence encoding.
!>   @param [in] MSG The message object of type `FORTRAN_MESSAGE_T` for communication during encoding.
!>   @param [in] CURR_TIME The current time object of type `CURR_TIME_T` for time-based encoding decisions.
!>   @param [in] TIME_HIST The time history object of type `TIME_HISTORY_T` providing historical time data.
!>   @param [inout] TO_BE_ENCODED Logical flag indicating if the GRIB section should be encoded.
...
!>
PP_THREAD_SAFE FUNCTION GRIB_SECTION_TO_BE_ENCODED_IF( THIS, PAR, &
...
!>
PP_THREAD_SAFE FUNCTION GRIB_SECTION_FREE_IF( THIS, HOOKS ) RESULT(RET)
...
!>   - `1`: Failure, an issue occurred during resource deallocation.
!>
PP_THREAD_SAFE FUNCTION GRIB_SECTION_PRINT_IF( THIS, UNIT, OFFSET, HOOKS ) RESULT(RET)
