...
FUNCTION FILTER_FREQUENCY_INIT( THIS, CFG, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  CASE( FLT_INT_MATCH_E )
    ...
    IF ( THIS%USE_KEYSET_ ) THEN
      PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_KEYSET_MATCH) THIS%FREQUENCIES_%MATCH( MSG%LEVEL(2), MATCH, HOOKS )
    ELSE
      MATCH = (THIS%FREQUENCY_ .EQ. MSG%LEVEL(2))
    ENDIF
  ...
  CASE( FLT_INT_IGNORE_E )
    ...
    IF ( THIS%USE_KEYSET_ ) THEN
      ...
      MATCH = .NOT.MATCH
    ELSE
      MATCH = .NOT.(THIS%FREQUENCY_ .EQ. MSG%LEVEL(2))
    ENDIF

  CASE( FLT_INT_GT_E )

    MATCH = (MSG%LEVEL(2) .GT. THIS%FREQUENCY_ )

  CASE( FLT_INT_GE_E )

    MATCH = (MSG%LEVEL(2) .GE. THIS%FREQUENCY_ )

  CASE( FLT_INT_LE_E )

    MATCH = (MSG%LEVEL(2) .LE. THIS%FREQUENCY_ )

  CASE( FLT_INT_LT_E )

    MATCH = (MSG%LEVEL(2) .LT. THIS%FREQUENCY_ )
...
IMPLICIT NONE
  ...
  WRITE(UNIT,*) REPEAT(' ',OFFSET)//'FREQUENCY FILTER: '//TRIM(ADJUSTL(OPERATION_TYPE_STR))
