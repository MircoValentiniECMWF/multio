...
FUNCTION FILTER_PARAM_INIT( THIS, CFG, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  CASE( FLT_INT_MATCH_E )
    ...
    IF ( THIS%USE_KEYSET_ ) THEN
      PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_KEYSET_MATCH) THIS%PARAMIDS_%MATCH( MSG%PARAM_ID, MATCH, HOOKS )
    ELSE
      MATCH = (THIS%PARAMID_ .EQ. MSG%PARAM_ID)
    ENDIF
  ...
  CASE( FLT_INT_IGNORE_E )
    ...
    IF ( THIS%USE_KEYSET_ ) THEN
      ...
      MATCH = .NOT.MATCH
    ELSE
      MATCH = .NOT.(THIS%PARAMID_ .EQ. MSG%PARAM_ID)
    ENDIF

  CASE( FLT_INT_GT_E )

    MATCH = (MSG%PARAM_ID .GT. THIS%PARAMID_ )

  CASE( FLT_INT_GE_E )

    MATCH = (MSG%PARAM_ID .GE. THIS%PARAMID_ )

  CASE( FLT_INT_LE_E )

    MATCH = (MSG%PARAM_ID .LE. THIS%PARAMID_ )

  CASE( FLT_INT_LT_E )

    MATCH = (MSG%PARAM_ID .LT. THIS%PARAMID_ )
...
IMPLICIT NONE
  ...
  WRITE(UNIT,*) REPEAT(' ',OFFSET)//'PARAM FILTER: '//TRIM(ADJUSTL(OPERATION_TYPE_STR))
