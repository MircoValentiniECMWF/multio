...
!>
!> This module defines the `FILTER_LEVEL_T` type, along with its associated
...
#define PP_FILE_NAME 'filter_level_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'FILTER_LEVEL_MOD'
MODULE FILTER_LEVEL_MOD
...
!>
TYPE, EXTENDS(FILTER_BASE_A) :: FILTER_LEVEL_T
  ...
  !> @details If `.TRUE.`, the `LEVELS_` keyset is used for filtering specific parameter IDs.
  LOGICAL :: USE_KEYSET_
  ...
  TYPE(KEYSET_INT64_T) :: LEVELS_

CONTAINS

  !> @brief Initializes the filter parameter type.
  !> @details This procedure sets up the `FILTER_LEVEL_T` type, initializing its components.
  !>
  !> @param [in] this The instance of `FILTER_LEVEL_T` to initialize.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: INIT  => FILTER_LEVEL_INIT
  ...
  !>
  !> @param [in] this The instance of `FILTER_LEVEL_T`.
  ...
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: MATCH => FILTER_LEVEL_MATCH

  !> @brief Prints the filter's details for debugging or logging.
  !> @details Outputs the filter's configuration and current state.
  !>
  !> @param [in] this The instance of `FILTER_LEVEL_T` to print.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: PRINT => FILTER_LEVEL_PRINT

  !> @brief Frees resources allocated for the filter.
  !> @details Cleans up the `FILTER_LEVEL_T` type, deallocating any resources used by the filter.
  !>
  !> @param [in] this The instance of `FILTER_LEVEL_T` to free.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: FREE  => FILTER_LEVEL_FREE
...
PUBLIC :: FILTER_LEVEL_T

CONTAINS


!>
!> @brief Initializes the `FILTER_LEVEL_T` filter from a YAML configuration.
!>
!> This function initializes the `FILTER_LEVEL_T` type based on the provided YAML
...
!>
!> @param [inout] THIS The filter object (`FILTER_LEVEL_T`) that will be initialized.
...
#define PP_PROCEDURE_NAME 'FILTER_LEVEL_INIT'
FUNCTION FILTER_LEVEL_INIT( THIS, CFG, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(FILTER_LEVEL_T),      INTENT(INOUT) :: THIS
  ...
  PP_TRYCALL(ERRFLAG_INIT_KEYSET) THIS%LEVELS_%INIT( HOOKS )
...
&          .NOT.HAS_TRESHOLD .AND. .NOT.HAS_VALUE ) THEN
    ...
    !> Read all the subconfigurations
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_VALUES) YAML_READ_INTEGER_KEYSET_WITH_RANGES( CFG, 'values', THIS%LEVELS_, HOOKS )
...
END FUNCTION FILTER_LEVEL_INIT
...
!>
!> @param [inout] THIS  The filter object (`FILTER_LEVEL_T`) used for matching.
...
FUNCTION FILTER_LEVEL_MATCH( THIS, MSG, PAR, MATCH, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  CASE( FLT_INT_MATCH_E )
    ...
    IF ( THIS%USE_KEYSET_ ) THEN
      PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_KEYSET_MATCH) THIS%LEVELS_%MATCH( MSG%LEVEL(1), MATCH, HOOKS )
    ELSE
      MATCH = (THIS%LEVEL_ .EQ. MSG%LEVEL(1))
    ENDIF
  ...
  CASE( FLT_INT_IGNORE_E )
    ...
    IF ( THIS%USE_KEYSET_ ) THEN
      ...
      MATCH = .NOT.MATCH
    ELSE
      MATCH = .NOT.(THIS%LEVEL_ .EQ. MSG%LEVEL(1))
    ENDIF

  CASE( FLT_INT_GT_E )

    MATCH = (MSG%LEVEL(1) .GT. THIS%LEVEL_ )

  CASE( FLT_INT_GE_E )

    MATCH = (MSG%LEVEL(1) .GE. THIS%LEVEL_ )

  CASE( FLT_INT_LE_E )

    MATCH = (MSG%LEVEL(1) .LE. THIS%LEVEL_ )

  CASE( FLT_INT_LT_E )

    MATCH = (MSG%LEVEL(1) .LT. THIS%LEVEL_ )
...
END FUNCTION FILTER_LEVEL_MATCH
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @param [inout] THIS    The filter object (`FILTER_LEVEL_T`) whose details are to be printed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_LEVEL_PRINT'
FUNCTION FILTER_LEVEL_PRINT( THIS, UNIT, OFFSET, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  WRITE(UNIT,*) REPEAT(' ',OFFSET)//'LEVEL FILTER: '//TRIM(ADJUSTL(OPERATION_TYPE_STR))
...
END FUNCTION FILTER_LEVEL_PRINT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @param [inout] THIS   The filter object (`FILTER_LEVEL_T`) whose resources are to be freed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_LEVEL_FREE'
FUNCTION FILTER_LEVEL_FREE( THIS, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Free keyset
  PP_TRYCALL(ERRFLAG_UNABLE_TO_FREE_KEYSET) THIS%LEVELS_%FREE( HOOKS )
...
END FUNCTION FILTER_LEVEL_FREE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE

END MODULE FILTER_LEVEL_MOD
