!>
!> @file grib2_section6_000_mod.F90
!>
!> @brief Module for managing GRIB2 Section 6 operations.
!>
!> The `GRIB2_SECTION6_000_MOD` module contains procedures to initialize, allocate,
!> preset, run, and clean up the resources associated with GRIB2 Section 6 objects.
...
!>   - Initialization of GRIB2 Section 6 objects.
...
!>   - @see GRIB2_SECTION6_000_INIT
!>   - @see GRIB2_SECTION6_000_ALLOCATE
!>   - @see GRIB2_SECTION6_000_PRESET
!>   - @see GRIB2_SECTION6_000_RUNTIME
!>   - @see GRIB2_SECTION6_000_TO_BE_ENCODED
!>   - @see GRIB2_SECTION6_000_FREE
!>
...
#define PP_FILE_NAME 'grib2_section6_000_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'GRIB2_SECTION6_000_MOD'
MODULE GRIB2_SECTION6_000_MOD
...
!>
!> @brief Type definition for GRIB2 Section 6 handler.
!>
!> The `GRIB2_SECTION6_000_T` type extends the base class `GRIB_SECTION_BASE_A` and
!> provides concrete implementations of initialization, allocation, preset, runtime,
!> encoding checks, and cleanup operations for GRIB2 Section 6 objects.
!>
...
!> environments.
!>
TYPE, EXTENDS(GRIB_SECTION_BASE_A) :: GRIB2_SECTION6_000_T

CONTAINS

  !>
  !> @brief Initializes the GRIB2 Section 6 object.
  !>
  !> This procedure sets up the necessary parameters and prepares the
  !> object for use.
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: INIT => GRIB2_SECTION6_000_INIT

  !>
  !> @brief Allocates resources for the GRIB2 Section 6 object.
  !>
  !> This procedure allocates memory and other necessary resources for
  !> the object based on provided parameters.
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: ALLOCATE => GRIB2_SECTION6_000_ALLOCATE

  !>
  !> @brief Presets the parameters of the GRIB2 Section 6 object.
  !>
  !> This procedure configures the internal parameters of the object
  !> before runtime execution.
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: PRESET => GRIB2_SECTION6_000_PRESET

  !>
  !> @brief Manages the runtime execution of GRIB2 Section 6 operations.
  !>
  !> This procedure handles operations and computations during runtime,
  !> making use of time and metadata information.
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: RUNTIME => GRIB2_SECTION6_000_RUNTIME

  !>
  !> @brief Determines if the GRIB2 Section 6 object needs to be encoded.
  !>
  !> This procedure checks whether the object should be encoded based
  !> on the provided parameters and internal state.
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: TO_BE_ENCODED => GRIB2_SECTION6_000_TO_BE_ENCODED

  !>
  !> @brief Frees resources allocated for the GRIB2 Section 6 object.
  !>
  !> This procedure deallocates resources and performs cleanup after
  !> the object has been used.
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: FREE => GRIB2_SECTION6_000_FREE
  ...
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: PRINT => GRIB2_SECTION6_000_PRINT
...
PUBLIC :: GRIB2_SECTION6_000_T

CONTAINS

!>
!> @brief Initializes GRIB2 Section 6 for a given object using the provided parameters.
!>
!> This function initializes a GRIB2 Section 6 object (`THIS`) using the provided model parameters (`PARAMS`)
...
!> @section interface
!>
!> @param [inout] THIS    GRIB2 Section 6 object to be initialized.
...
#define PP_PROCEDURE_NAME 'GRIB2_SECTION6_000_INIT'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION6_000_INIT( THIS, PAR, &
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(GRIB2_SECTION6_000_T),  INTENT(INOUT) :: THIS
...
END FUNCTION GRIB2_SECTION6_000_INIT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Allocates resources for GRIB2 Section 6 using the provided parameters.
!>
!> This function allocates resources for a GRIB2 Section 6 object (`THIS`) using the provided model parameters (`PARAMS`),
...
!> @section interface
!>
!> @param [in]    THIS      GRIB2 Section 6 object for which resources are allocated.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION6_000_ALLOCATE'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION6_000_ALLOCATE( THIS, PAR, &
...
END FUNCTION GRIB2_SECTION6_000_ALLOCATE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Presets GRIB2 Section 6 using the provided parameters and message data.
!>
!> This function presets a GRIB2 Section 6 object (`THIS`) using the provided model parameters (`PARAMS`),
!> message structure (`MSG`), and metadata (`METADATA`). The process can be run in verbose mode if specified.
!> The function is thread-safe and returns an error code indicating the success or failure of the preset operation.
!>
!> @section interface
!>
!> @param [in]    THIS      GRIB2 Section 6 object to be preset.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION6_000_PRESET'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION6_000_PRESET( THIS, PAR, &
&   MSG, METADATA, HOOKS ) RESULT(RET)
...
END FUNCTION GRIB2_SECTION6_000_PRESET
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Executes runtime processing for GRIB2 Section 6 using provided parameters, message data, and time history.
!>
!> This function performs runtime operations for a GRIB2 Section 6 object (`THIS`) using the provided model parameters (`PARAMS`),
...
!> @section interface
!>
!> @param [in]    THIS          GRIB2 Section 6 object for runtime execution.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION6_000_RUNTIME'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION6_000_RUNTIME( THIS, PAR, &
...
END FUNCTION GRIB2_SECTION6_000_RUNTIME
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Prepares GRIB2 Section 6 for encoding based on provided parameters, message data, and time history.
!>
!> This function determines whether GRIB2 Section 6 (`THIS`) is ready to be encoded. It processes the provided model parameters
...
!> @section interface
!>
!> @param [inout] THIS          GRIB2 Section 6 object to be checked for encoding readiness.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION6_000_TO_BE_ENCODED'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION6_000_TO_BE_ENCODED( THIS, PAR, &
...
END FUNCTION GRIB2_SECTION6_000_TO_BE_ENCODED
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Frees resources associated with GRIB2 Section 6 object.
!>
!> This function deallocates and cleans up resources associated with the GRIB2 Section 6 object (`THIS`).
...
!> @section interface
!>
!> @param [inout] THIS    GRIB2 Section 6 object to be deallocated and freed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION6_000_FREE'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION6_000_FREE( THIS, HOOKS ) RESULT(RET)
...
END FUNCTION GRIB2_SECTION6_000_FREE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION6_000_PRINT'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION6_000_PRINT( THIS, UNIT, OFFSET, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  IF ( OFFSET .LE. 0 ) THEN
    WRITE(UNIT,'(A,A,A,A)', IOSTAT=WRITE_STAT) 'GRIB2_SECTION6_000_T: ', TRIM(ADJUSTL(THIS%NUMBER_)), '::', TRIM(ADJUSTL(THIS%TYPE_))
    PP_DEBUG_CRITICAL_COND_THROW(WRITE_STAT.NE.0, ERRFLAG_WRITE_ERROR )
  ELSE
    WRITE(UNIT,'(A,A,A,A,A)', IOSTAT=WRITE_STAT) REPEAT(' ',OFFSET), 'GRIB2_SECTION6_000_T: ', TRIM(ADJUSTL(THIS%NUMBER_)), '::', TRIM(ADJUSTL(THIS%TYPE_))
    PP_DEBUG_CRITICAL_COND_THROW(WRITE_STAT.NE.0, ERRFLAG_WRITE_ERROR )
  ENDIF
...
END FUNCTION GRIB2_SECTION6_000_PRINT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


END MODULE GRIB2_SECTION6_000_MOD
