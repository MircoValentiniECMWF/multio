...
PP_THREAD_SAFE FUNCTION MAKE_ENCODER( ENCODER, ID, PAR, &
...
IMPLICIT NONE
  ...
  PP_TRYCALL(ERRFLAG_INITIALIZATION_ERROR)  ENCODER%INIT( PAR, CFG, OPT, HOOKS )
...
PP_THREAD_SAFE FUNCTION DESTROY_ENCODER( ENCODER, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Free the section
  PP_TRYCALL(ERRFLAG_FREE_ERROR)  ENCODER%FREE( HOOKS )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_DEALLOCATION_ERROR)
      ...
      ELSE
        PP_DEBUG_PUSH_MSG_TO_FRAME( 'error deallocating ENCODER number: '//TRIM(ADJUSTL(ERRMSG)) )
        DEALLOCATE(ERRMSG)
      ENDIF
