...
IFS=$'\n'
for i in `cat type_of_section4_time.txt` ; do
  UNAME=`echo $i | awk '{print $1}'`
  LNAME=`echo $i | awk '{print $1}' |sed -e 's/.*/\L&/'`
  FNAME="grib2_section4_time_${LNAME}_mod.F90"
  echo $LNAME
  echo $UNAME
  echo $FNAME
  rm -f $FNAME
  cat grib2_section4_time_XXX_mod.F90 | sed "s/@XXX@/${UNAME}/g" | sed "s/@YYY@/${LNAME}/g" > ./$FNAME
done
