...
CONTAINS
  ...
  !> GRIB2 encoder.
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: INIT_CFG => GRIB2_SECTION2_@XXX@_INIT_CFG
  ...
  !> the GRIB2 encoder.
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: INIT_LAZY => GRIB2_SECTION2_@XXX@_INIT_LAZY
...
!>
!> @section interface
!>   @param [in]    THIS     An object of type `GRIB2_SECTION2_@XXX@_T` representing the GRIB section to be preset.
...
PP_THREAD_SAFE FUNCTION GRIB2_SECTION2_@XXX@_PRESET( THIS, &
&  MSG, PAR, OPT,  METADATA, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  PP_METADATA_SET( METADATA, 'expver', MSG%EXPVER )
...
!>
!> @section interface
!>   @param [in]    THIS      An object of type `GRIB2_SECTION2_@XXX@_T` representing the GRIB section for runtime execution.
!>   @param [in]    MSG       The message object of type `FORTRAN_MESSAGE_T` used to handle preset-related messaging.
!>   @param [in]    PAR       The parametrization structure of type `PARAMETRIZATION_T` used for the preset operation.
...
PP_THREAD_SAFE FUNCTION GRIB2_SECTION2_@XXX@_TO_BE_ENCODED( THIS, &
