...
# Load modules
module_load cdt/18.10
module_load gcc/6.3.0
export FFTW_PATH=$( { module load fftw/3.3.4.5; } 2>/dev/null; echo $FFTW_DIR )
export NETCDF_ROOT=$( { module load netcdf4/4.4.1; } 2>/dev/null; echo $NETCDF_DIR )
export HDF5_ROOT=$( { module load cray-hdf5/1.8.17; } 2>/dev/null; echo $HDF5_DIR )
