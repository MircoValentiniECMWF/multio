...
# Load modules
module_load cdt/17.09
module_load gcc/6.3.0
export FFTW_PATH=$( { module load fftw/3.3.4.5; } 2>/dev/null; echo $FFTW_DIR )
export NETCDF_ROOT=$( { module load netcdf4; } 2>/dev/null; echo $NETCDF4_DIR )
export HDF5_ROOT=$( { module load cray-hdf5; } 2>/dev/null; echo $HDF5_DIR )
